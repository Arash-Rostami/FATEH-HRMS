...
A single global Alpine store backs the outgoing (send) chat sound and the per-chat mute for both the contact and channel modules. Registered like the other stores (`import registerSoundStore from './stores/sound.js'` + `registerSoundStore(Alpine)` in `main.js`); auto-inits via `init()`.

**Two scopes, disjoint id spaces:** `mutedChannels: []` + `mutedContacts: []` (array-backed, NOT `Set` — Alpine reacts to `Array` `splice`/`push` but not `Set.has`, so the header/sidebar bindings update live on toggle), persisted to `localStorage['chat-muted-channels']` / `localStorage['chat-muted-contacts']`. Every method takes a `scope = 'channel'` arg (`_set`/`_key` pick the list/storage-key); contact callers pass `'contact'`. Channel ids and contact user ids are disjoint, so two sets — never one.

**One sound, one cached `Audio`:** `_audio: null` holds a single `Audio` element, lazily built on first play from `document.querySelector('[data-outgoing-sound]')?.dataset.outgoingSound` and cached (NOT a fresh `new Audio` per play — `currentTime = 0` restarts the cached element, so no per-send allocation/GC). `volume = 0.35`; `play().catch(() => {})` (send is a user gesture → autoplay unlocked; the catch swallows any rejection). One `data-outgoing-sound` attr on each module root (`channel.blade.php`, `contact.blade.php`): `{{ asset('build/assets/audio/outgoing.mp3') }}` — the URL-in-view precedent (same as `stopwatch.blade.php`'s `alarm.mp3`), no inline `<script>`. Asset copied by `vite-plugin-static-copy` (whole `resources/assets/audio` dir → `public/build/assets/audio`); standard name `outgoing.mp3`.

**Methods:** `isMuted(id, scope)` / `toggleMute(id, scope)` (push/splice + persist, `try/catch`-wrapped); `playOutgoing(id, scope)`.

**`playOutgoing` contract (load-bearing rules):**
- `id !== null && isMuted(id, scope)` → early return (per-chat mute; the active chat's id is passed so a muted chat is silent).
- `document.hidden` → early return (no sound on an inactive tab).
- URL + `Audio` resolved lazily on first play (the module root isn't in the DOM at `alpine:init`). Only one chat module is mounted at a time; both set the same URL, so the cache is valid across module switches.

**Caller-side fault isolation (mandatory):** the `message-sent` handlers call `this.$store.sound?.playOutgoing(...)` with **optional chaining**, ordered **after** the critical UI state (`scrollToBottom` + `sending = false`) — the sound is non-essential and must never strand the composer in `sending = true`. `$store.*` resolves inside `@island` (channel header/sidebar mute live in islands) because stores are global, not scope-bound.

**No incoming (receive) sound:** incoming detection was evaluated and dropped as too costly/risky for the minimal gain. The contact side is blocked by Livewire's snapshot model — `#[Computed]` values (`lastMessageId`/`lastMessageIsMine`) are NOT serialized into the snapshot (only public properties are, via `getPublicPropertiesDefinedOnSubclass`), so a `$watch('$wire.lastMessageId')` never fires on the client; a correct fix needs a `Livewire.hook('morph')` + per-row `data-mine` + switch/seed guards. The channel side is feasible (sidebar-poll piggyback reading `data-last-at`/`data-last-mine` off the active row) but would give inconsistent UX against a silent contact module. Outgoing-only is the deliberate minimal choice — the send sound + per-chat mute is the kept surface.

**Mute UI (both modules):**
- **Header toggle** (`channel/header.blade.php`, `contact/header.blade.php`): first control in the actions cluster, `:class="… ? '!bg-primary !text-on-primary' : 'hover:brightness-95'"` active/idle vocabulary (muted = primary fill). Glyph swaps via Alpine `x-text` (`volume_off`/`volume_up`) — NOT Blade `{{ }}`. `aria-pressed` carries state; `aria-label`/`title` carry the **next action**. Channel uses `$header['id']`; contact uses `$activeContact->id` + `'contact'` scope. Mute is client-only — no server round-trip, no IDOR surface.
- **Channel sidebar hover shortcut** (`channel/sidebar.blade.php`): the channel row is a `<div role="option" tabindex="0">` (NOT a `<button>`) with `x-on:keydown.enter/space.prevent` → `selectChannel`, because a real mute `<button>` is nested inside it — and `<button>` content model forbids a `tabindex`/interactive descendant. The nested mute `<button>` (hover/focus-revealed via `opacity`/`scale` + `pointer-events-none` when idle / `pointer-events-auto` on `group-hover`/`focus`/muted) carries `x-on:click.stop` + `x-on:keydown.enter.stop`/`x-on:keydown.space.stop` — the `.stop` (no `.prevent`) stops the keydown bubbling to the row's `selectChannel` handler WITHOUT suppressing the button's native synthesized click, so keyboard Enter/Space mutes (not selects). `pointer-events-none` on the idle/hidden state is mandatory: without it the invisible 24px target intercepts taps on touch devices (no `group-hover`), toggling mute instead of opening the channel. The icon `<span>` is `aria-hidden="true"` (the header button is the keyboard-accessible control; the sidebar span is a pointer shortcut). The contact sidebar (`contact/sidebar.blade.php`) now mirrors this exactly: its row was converted from `<button wire:click="selectContact(...)">` to `<div role="option" tabindex="0" x-on:click="$wire.selectContact(...)" x-on:keydown.enter.prevent="$wire.selectContact(...)" x-on:keydown.space.prevent="$wire.selectContact(...)">` so the same per-row hover mute `<button>` (scope `'contact'`, labels 'بی‌صدا کردن مخاطب'/'باصدا کردن مخاطب') can nest inside — `<button>` cannot nest a `<button>`, and the contact row is a plain `@include` (NOT an `@island` like channel), so there is no Alpine `selectChannel`-style wrapper; the row calls `$wire.selectContact` directly via `x-on:click` (the exact equivalent of the old `wire:click`), with `enter`/`space` `.prevent` keyboard handlers added so the `<div>` keeps the activation the native `<button>` used to provide. `wire:key` stays on the element (works on any tag); `group` is added to the row's `@class` so the mute button's `group-hover:` reveal fires.
- **Sidebar mute-all button (both modules):** `isAllMuted(ids, scope)` / `toggleAll(ids, scope)` added to the sound store — array-populating bulk mute that writes the SAME `mutedChannels`/`mutedContacts` arrays per-item uses, so the two compose: `toggleAll` mutes-all by pushing every missing id (or un-mutes-all by reverse-iterating the list and splicing every id present — backward iteration avoids the index-shift bug of splicing while forward-iterating); a per-item `toggleMute` that removes one id then flips `isAllMuted` false, so the mute-all button auto-reverts to idle without a second store. `playOutgoing` is unchanged (it already consults `isMuted`, which is true for every id once `toggleAll` populates the array). The button lives in each sidebar's title-row right cell, wrapped in `flex items-center gap-1.5` alongside the unread badge (badge first, button last/rightmost), guarded by `@if(count($list))` so an empty/filtered-out list shows no button. It is 24px (`w-6 h-6` — sidebar scale, intentionally smaller than the 28px `min-w-[28px] min-h-[28px]` header action cluster, which itself shrank from 40px `min-w-10 min-h-10` with glyphs 18→16px to hold ~57% fill — the header action glyphs ALL scale with the box so no icon outsizes its button; the per-row hover mute is the same 24px `w-6 h-6` with a 14px glyph, ~58% fill, matching the header ratio) and mirrors the header mute vocabulary: primary fill + `volume_off` when all-muted, `surface-variant` + `hover:brightness-95` + `volume_up` when not, `aria-pressed = isAllMuted`, `aria-label`/`title` = the next action ('بی‌صدا کردن همه …' / 'باصدا کردن همه …'), glyph via `x-text`. The rendered ids are passed as an inline JSON array literal — `$allChannelIds = collect($channelList)->pluck('id')->map(fn($id) => (int) $id)->values()->toJson()` in the sidebar `@php` block, then `{{ $allChannelIds }}` in the binding — which is a non-string-literal function argument, so it does NOT trip the quoting pitfall, and it re-evaluates on Livewire poll/morph exactly like the existing `:class` bindings that interpolate `{{ $contact['unread'] }}`. Channel scope `'channel'` / ids `$allChannelIds`; contact scope `'contact'` / ids `$allContactIds`. Client-only — no server round-trip, no IDOR.
