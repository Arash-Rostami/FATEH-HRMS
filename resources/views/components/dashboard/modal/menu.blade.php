                    ...
                    <div class="relative px-5 py-5 bg-[var(--md-sys-color-primary)] border-b border-white/5 shrink-0">
                        ...
                        <div class="flex items-center gap-3">
                            ...
                            <div class="w-14 h-14 rounded-2xl bg-[var(--md-sys-color-primary-container)] flex items-center justify-center shadow-md overflow-hidden shrink-0">
                                ...
                                @else
                                    <span class="material-symbols-rounded text-[26px] text-[var(--md-sys-color-on-primary-container)]">account_circle</span>
                                @endif
                            </div>
                            <!-- Text -->
                                                    ...
                                                    <template x-for="item in pageItems(p-1)" :key="item.href">
                                                        <a :href="item.href" @click="closeMenu"
                                                           class="group flex flex-col items-center gap-2 p-3 rounded-2xl bg-[var(--md-sys-color-surface-container-low)] hover:bg-[var(--md-sys-color-surface-container)] active:scale-[0.96] transition-all duration-200 h-[110px] sm:h-[130px] justify-center border border-transparent hover:border-[var(--md-sys-color-outline-variant)]/20">
                                                            <div class="w-10 h-10 sm:w-12 sm:h-12 rounded-xl flex items-center justify-center bg-gradient-to-tr from-[var(--md-sys-color-primary)]/10 to-[var(--md-sys-color-primary)]/5 border border-[var(--md-sys-color-primary)]/10 shadow-sm group-hover:shadow-md group-hover:scale-110 transition-all duration-300">
                                                                <span class="material-symbols-rounded text-[22px] sm:text-[24px] text-[var(--md-sys-color-primary)]"
                                                                      x-text="item.icon"></span>
                                                            </div>
                                                            <div class="text-center w-full">
                                                                <div class="text-[11px] sm:text-sm font-bold text-[var(--md-sys-color-on-surface)] mb-0.5 truncate px-1"
                                                                     x-text="item.title"></div>
                                                                <div class="text-[10px] text-[var(--md-sys-color-on-surface-variant)] hidden sm:block leading-tight truncate px-1 opacity-80"
                                                                     x-text="item.sub"></div>
                                                            </div>
                                                        </a>
                                                    </template>
