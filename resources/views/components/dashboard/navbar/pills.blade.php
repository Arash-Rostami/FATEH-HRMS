...
@if($user?->profile)
    <div {{ $attributes->merge(['class' => 'hidden xl:flex items-center gap-1']) }}>
        ...
        {{-- Birthday Badge --}}
        <div class="relative w-10 h-10 rounded-full flex items-center justify-center cursor-help group overflow-visible transition-all duration-200">
            ...
            <span class="absolute top-1 right-0 translate-x-1/2 -translate-y-1/2 min-w-[18px] h-4 px-1.5 rounded-full text-white text-[9px] font-bold flex items-center justify-center">
            ...
            <x-dashboard.tooltip text="روزهای باقیمانده تا تولد" position="bottom" />
        </div>
        ...
        <div class="relative w-10 h-10 rounded-full flex items-center justify-center cursor-help group overflow-visible transition-all duration-200">
            ...
            <x-dashboard.tooltip text="سابقه همکاری (روز)" position="bottom" />
        </div>
