@php
    ...
    $sections = [
        'workspace' => [
            'intro' => 'هر پروژه یک فضای کاری خصوصی با هفت برگه است؛ فقط سازنده و اعضای افزوده‌شده آن را می‌بینند.',
            'rows' => [
                ['icon' => 'space_dashboard', 'color' => 'primary', 'label' => 'یک فضای کاری، هفت برگه', 'text' => 'چرخهٔ کاری، فعالیت‌ها و نظرات، چت زندهٔ تیم، تقویم، برد وظایف، گزارش و تحلیل‌ها همگی در همین صفحه‌اند — نیازی به رفتن به ماژول جداگانه نیست. با بازکردن یک پروژه، برگهٔ «چرخهٔ کاری» به‌صورت پیش‌فرض باز می‌شود.'],
                ...
                ['icon' => 'bookmark_add', 'color' => 'tertiary', 'label' => 'الگوی چرخهٔ کاری قابل استفادهٔ مجدد', 'text' => 'یک چرخهٔ کاری ساخته‌شده را می‌توان به‌عنوان الگو ذخیره و در پروژه‌های دیگر دوباره اعمال کرد؛ چرخهٔ تمام‌شده یا لغوشده را هم می‌توان بدون الگوی جداگانه، با «دوباره اجرا کن» از نو شروع کرد.'],
                ['icon' => 'task_alt', 'color' => 'primary', 'label' => 'مرحله‌ای که به وظیفهٔ واقعی وصل است', 'text' => 'چنین مرحله‌ای دکمهٔ تکمیل جداگانه ندارد — با انجام و (در صورت نیاز) تأیید همان وظیفه در برد وظایف، چرخه خودکار به مرحلهٔ بعد می‌رود.'],
                ['icon' => 'shield_person', 'color' => 'secondary', 'label' => 'چه کسی می‌تواند هر مرحله را جلو ببرد', 'text' => 'فقط مسئولان همان مرحله یا مالک چرخه می‌توانند آن را تکمیل، تأیید یا رد کنند؛ اگر مرحله‌ای گیر کرد (مثلاً وظیفهٔ متصلش حذف شد)، فقط مالک با «پیشروی اجباری» می‌تواند آن را دور بزند.'],
                ['icon' => 'layers', 'color' => 'tertiary', 'label' => 'چند چرخهٔ هم‌زمان روی یک پروژه', 'text' => 'هر پروژه می‌تواند چند چرخهٔ کاری فعال هم‌زمان داشته باشد؛ شروع چرخهٔ تازه، چرخه‌های دیگرِ در حال اجرا را متوقف نمی‌کند.'],
            ],
        ],
        'comms' => [
        ...
        'planning' => [
            ...
            'rows' => [
                ...
                ['icon' => 'palette', 'color' => 'secondary', 'label' => 'نقطه‌های رنگی روی هر روز', 'text' => 'سبز=ایجاد وظیفه، زرد=تغییر وضعیت، قرمز=مهلت باز، آبی=تکمیل؛ مهلتی که وظیفه‌اش انجام یا آرشیو شده به‌جای حذف، خاکستری می‌ماند. پرچم آبی‌کمرنگ (مهلت پروژه) روزِ سقف مهلتِ خود پروژه را نشان می‌دهد. آیکون «مسیر تبدیل» آغاز یا پایان چرخهٔ کاری پروژه را نشان می‌دهد (بنفش=آغاز، سبز=پایان، لغوشده کم‌رنگ‌تر دیده می‌شود)؛ چند چرخه در یک روز با «+عدد» جمع می‌شوند. با کلیک روی روز، جزئیات هر رویداد (و مدت انتظار در صورت وجود) پایین تقویم باز می‌شود؛ راهنمای رنگ‌ها با آیکون پالت کنار عنوان ماه.'],
    ...
    $notes = [
        ...
        'در فهرست کنار، حلقهٔ رنگی کنار نام هر پروژه درصد تکمیل وظایف آن را نشان می‌دهد و در پایان سبز می‌شود. اگر پروژه چرخهٔ کاری فعال داشته باشد، نشان کوچک «مسیر تبدیل» کنار همان حلقه هم دیده می‌شود — با یک چرخهٔ فعال، شمارهٔ مرحلهٔ جاری از کل (مثلاً ۲/۳)؛ با چند چرخهٔ هم‌زمان، فقط تعدادشان (مثلاً ×۲).',
    ...
    $groups = [
        'workspace' => [
            ...
            ['id' => 'cycle', 'icon' => 'conversion_path', 'label' => 'چرخهٔ کاری', 'rows' => [$w[6], $w[7], $w[8], $w[9]]],
        ],
        'comms' => [
        ...
        'planning' => [
            ['id' => 'calendar', 'icon' => 'today', 'label' => 'تقویم', 'rows' => [$p[0], $p[1]]],
            ['id' => 'markers', 'icon' => 'palette', 'label' => 'نشانه‌های رنگی', 'rows' => [$p[3]]],
            ['id' => 'gantt', 'icon' => 'view_timeline', 'label' => 'گانت و دیرکرد', 'rows' => [$p[4], $p[5]]],
            ['id' => 'board', 'icon' => 'view_kanban', 'label' => 'برد وظایف', 'rows' => [$p[2], $p[6], $p[7], $p[8]]],
        ],
        'report' => [
