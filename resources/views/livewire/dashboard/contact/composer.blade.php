<footer class="flex-shrink-0 border-t backdrop-blur-sm bg-[color-mix(in_srgb,var(--md-sys-color-surface)_90%,transparent)] border-[color-mix(in_srgb,var(--md-sys-color-outline-variant)_30%,transparent)]">
    ...
    <div class="relative">
        ...
        <div class="flex flex-wrap items-center gap-1.5 px-4 py-2.5">
            <div x-data="{ len: 0 }"
                ...
                <textarea id="msg-ta" dir="rtl"
                          ...
                          x-on:input="len = $el.value.length; autoGrow($el, 160); autoDirection($el); resetPollInterval(); pingTyping()"
            ...
            <div x-data="{ open: false, pos: {} }">
                ...
                <template x-teleport="body">
                    <div x-show="open" x-cloak dir="rtl"
                        ...
                        <button type="button" x-on:click="open = false; document.getElementById('composer-attachments').click()"
                                class="w-full flex items-center gap-2 px-2 py-1.5 rounded-lg text-xs font-medium hover:bg-[var(--md-sys-color-surface-container-high)]/50">
        ...
        <p class="hidden md:grid grid-cols-3 items-center pb-2.5 px-4 text-[10px] text-[var(--md-sys-color-on-surface-variant)] opacity-60" aria-hidden="true">
            <span class="justify-self-start" x-text="(2000 - (($wire.composer.body || '').length)) + ' کاراکتر باقی‌مانده'"></span>
