<template x-teleport="body">
    <div
        ...
        "
        class="fixed inset-0 z-[100] flex items-center justify-center px-3 sm:px-0 bg-[var(--md-sys-color-primary)]/60 animate-slide-down"
        style="display: none;"
        x-show="show"
        dir="rtl"
    >
        ...
        <div class="absolute inset-0 bg-[var(--md-sys-color-primary)]/60"
             @click=" setTimeout(() => show = false, 300)"></div>


        <!-- Content -->
        <div
            class="bg-[var(--md-sys-color-surface-container-high)] w-full max-w-6xl h-[90vh] md:h-[95vh] rounded-2xl shadow-[0_8px_32px_rgba(0,0,0,0.12)] border border-[var(--md-sys-color-outline-variant)]/30 flex flex-col overflow-hidden"
            :class="active ? 'translate-y-0' : 'translate-y-0'"
            @click.away="show = false">

            <!-- Header -->
            <div
                class="shrink-0 h-16 px-6 flex items-center justify-between border-b border-[var(--md-sys-color-outline-variant)]/50 bg-[var(--md-sys-color-surface)]">
                <div class="flex items-center gap-3">
                    <div
                        class="w-10 h-10 rounded-xl bg-[var(--md-sys-color-primary-container)] text-[var(--md-sys-color-on-primary-container)] flex items-center justify-center shrink-0">
                        <span class="material-symbols-rounded text-[22px]"
                              x-text="isPdf ? 'picture_as_pdf' : 'description'"></span>
                    </div>
                    <h3 class="text-lg font-bold text-[var(--md-sys-color-on-surface)] truncate max-w-sm md:max-w-xl">
                        مشاهده و تأیید سند</h3>
                </div>

                <button @click="show = false"
                        class="w-10 h-10 rounded-lg flex items-center justify-center text-[var(--md-sys-color-on-surface-variant)] hover:bg-[var(--md-sys-color-surface-variant)] hover:text-[var(--md-sys-color-on-surface)] transition-colors group">
                    <span
                        class="material-symbols-rounded text-[22px] group-hover:rotate-90 transition-transform duration-300">close</span>
                </button>
            </div>
            ...
            <div class="flex-1 w-full bg-[#525659] relative overflow-hidden">
                ...
                <template x-if="show && !isPdf">
                    <div
                        ...
                        <div
                            class="w-24 h-24 rounded-3xl bg-[var(--md-sys-color-surface-variant)] text-[var(--md-sys-color-on-surface-variant)] flex items-center justify-center mb-6 shadow-sm">
            ...
            <!-- Footer / Action Area -->
            <div
                class="shrink-0 p-5 bg-[var(--md-sys-color-surface)] border-t border-[var(--md-sys-color-outline-variant)]/50 flex flex-col md:flex-row items-center justify-between gap-4">
                <div
