    ...
    <div class="p-4 flex items-center justify-between border-b border-[var(--md-sys-color-outline-variant)]/10 bg-[var(--md-sys-color-surface-container)]/50 backdrop-blur-sm">
        <div class="flex items-center gap-3">
            <div class="relative w-10 h-10 rounded-full overflow-hidden border border-[var(--md-sys-color-outline)]/20 shadow-sm">
                <img src="{{ $feed->user->profile_photo_url }}" alt="{{ $feed->user->name }}" class="w-full h-full object-cover">
            ...
            <div class="flex flex-col">
                <span class="text-sm font-bold text-[var(--md-sys-color-on-surface)]">{{ $feed->user->name }}</span>
                <span class="text-[10px] text-[var(--md-sys-color-on-surface-variant)]">{{ $feed->created_at->diffForHumans() }}</span>
            </div>
        </div>
