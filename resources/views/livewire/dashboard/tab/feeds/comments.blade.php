                ...
                <div class="flex-1 min-w-0">
                    <div class="bg-[var(--md-sys-color-surface-container-low)] rounded-3xl rounded-tr-none px-4 py-3 text-sm text-[var(--md-sys-color-on-surface)] relative group-hover:bg-[var(--md-sys-color-surface-container)] transition-all duration-300 shadow-sm hover:shadow-md">
                        ...
                        <div class="mt-2 flex items-center justify-end gap-3 text-[10px] text-[var(--md-sys-color-on-surface-variant)] opacity-70 group-hover:opacity-100 transition-opacity">
                             <button @click="replyingTo = replyingTo === {{ $comment->id }} ? null : {{ $comment->id }}" class="hover:text-[var(--md-sys-color-primary)] font-bold transition-colors">پاسخ</button>

                             @if(auth()->check() && auth()->id() === $comment->user_id)
                                <button wire:click="startEditing({{ $comment->id }})" class="hover:text-[var(--md-sys-color-primary)] transition-colors">ویرایش</button>
                                <button wire:click="deleteComment({{ $comment->id }})" class="hover:text-[var(--md-sys-color-error)] transition-colors">حذف</button>
                            @endif
                        </div>
                    </div>

                    <!-- Reply Form -->
                    <div x-show="replyingTo === {{ $comment->id }}" x-collapse class="mt-2 pl-2" style="display: none;">
                         <div class="relative group">
                            <input
                                type="text"
                                wire:model="replyComments.{{ $comment->id }}"
                                wire:keydown.enter="addComment({{ $feed->id }}, {{ $comment->id }}); replyingTo = null"
                            ...
                            >
                            <button
                                wire:click="addComment({{ $feed->id }}, {{ $comment->id }}); replyingTo = null"
                    ...
                    @if($comment->children && $comment->children->isNotEmpty())
                        <div class="mr-4 border-r border-[var(--md-sys-color-outline-variant)]/20 pr-3">
                            @include('livewire.dashboard.tab.feeds.comments', ['comments' => $comment->children, 'feed' => $feed, 'isNested' => true])
                        </div>
                    @endif
                </div>
            </div>
