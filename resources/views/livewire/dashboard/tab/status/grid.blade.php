        ...
        @forelse($this->users as $user)
            ...
            <div wire:key="user-{{ $user->id }}"
                 ...
                 class="group relative flex flex-col items-center gap-2 p-3 pt-4
                     ...
                     {{ $p === \App\Enums\PresenceStatus::Angry ? 'hover:-translate-y-1 animate-pulse' : 'hover:-translate-y-0.5' }}
                     {{ $p->cardClasses() }}">

                <div class="relative z-10 mt-1 {{ in_array($p, [\App\Enums\PresenceStatus::Grumpy, \App\Enums\PresenceStatus::Angry]) ? 'blur-[1px] grayscale-[50%]' : '' }} transition-all duration-300 group-hover:blur-none group-hover:grayscale-0">
                    <img
                        ...
                        class="w-18 h-18 rounded-full object-cover
                           ...
                           transition-all duration-300 {{ in_array($p, [\App\Enums\PresenceStatus::Busy]) ? 'animate-pulse' : '' }} {{ $p === \App\Enums\PresenceStatus::Angry ? 'drop-shadow-[0_0_10px_rgba(220,38,38,0.8)]' : '' }}"
                        loading="lazy"
                    >
                    ...
                    <div class="absolute -bottom-1 -right-1 w-5 h-5 rounded-full
                            ...
                            border-2 border-[var(--md-sys-color-surface)] shadow-sm {{ $p === \App\Enums\PresenceStatus::Angry ? 'animate-bounce' : '' }}">
                        <span
                            class="material-symbols-rounded text-white leading-none text-[10px]">{{ $p->icon() }}</span>
                    </div>
                </div>

                <div class="w-full text-center px-1 z-10 space-y-0.5 {{ in_array($p, [\App\Enums\PresenceStatus::Grumpy, \App\Enums\PresenceStatus::Angry]) ? 'blur-[2px] opacity-60' : '' }} transition-all duration-300 group-hover:blur-none group-hover:opacity-100">
                ...
                <div class="absolute inset-x-0 bottom-0 h-10 z-20
                        ...
                        transition-all duration-300 ease-[cubic-bezier(0.34,1.56,0.64,1)]
                        {{ in_array($p, [\App\Enums\PresenceStatus::Grumpy, \App\Enums\PresenceStatus::Angry]) ? 'opacity-0 pointer-events-none' : '' }}">

                    @if($user->sms_number && !in_array($p, [\App\Enums\PresenceStatus::Grumpy, \App\Enums\PresenceStatus::Angry]))
                        <button
                    ...
                    <div class="flex items-center gap-0.5 text-{{ $p->color() }}-400">
                        @if($user->getTodaysDeskExtension() && !in_array($p, [\App\Enums\PresenceStatus::Grumpy, \App\Enums\PresenceStatus::Angry]))
                        ...
                        @elseif(!$user->sms_number || in_array($p, [\App\Enums\PresenceStatus::Grumpy, \App\Enums\PresenceStatus::Angry]))
                            <span class="text-[10px] text-[var(--md-sys-color-outline)]">---</span>
                        @endif
                    </div>

                    @if(($user->sms_number || $user->getTodaysDeskExtension()) && !in_array($p, [\App\Enums\PresenceStatus::Grumpy, \App\Enums\PresenceStatus::Angry]))
