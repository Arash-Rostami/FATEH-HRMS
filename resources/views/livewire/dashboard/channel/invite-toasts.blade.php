<div class="fixed bottom-4 right-4 left-4 sm:left-auto z-50 flex flex-col gap-3 w-auto sm:w-full sm:max-w-[360px]" x-show="inviteToasts.length > 0" x-cloak>
    <template x-for="t in inviteToasts" :key="t.id">
        <div class="bg-[color-mix(in_srgb,var(--md-sys-color-surface)_92%,transparent)] dark:bg-[var(--md-sys-color-surface)] border border-[color-mix(in_srgb,var(--md-sys-color-outline-variant)_60%,transparent)] shadow-[0_8px_32px_color-mix(in_srgb,var(--md-sys-color-primary)_25%,transparent)] dark:shadow-[0_12px_40px_rgba(0,0,0,0.6)] rounded-2xl p-4 sm:p-5"
            ...
            <div class="flex items-start gap-3">
                <span class="material-symbols-rounded text-[22px] text-[var(--md-sys-color-primary)] flex-shrink-0 mt-0.5">group_add</span>
                <div class="min-w-0 flex-1">
                    <p class="text-[12px] font-medium text-[var(--md-sys-color-on-surface-variant)] leading-tight">شما به کانال زیر اضافه شدید</p>
                    <p class="text-[14px] font-bold text-[var(--md-sys-color-on-surface)] truncate mt-1" x-text="t.name"></p>
                </div>
            </div>
