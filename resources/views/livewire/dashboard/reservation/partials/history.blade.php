    ...
    <div
        ...
        @forelse($this->historyReservations as $reservation)
            <div wire:key="history-{{ $reservation->id }}"
                ...
                <div
                    class="flex items-center gap-3 sm:gap-4 pr-2 min-w-0 flex-1 {{ $activeHistoryTab === 'cancelled' ? 'opacity-70' : '' }}">
                    <div
                        class="w-10 h-10 sm:w-12 sm:h-12 rounded-xl {{ $activeHistoryTab === 'upcoming' ? 'bg-[var(--md-sys-color-primary-container)] text-[var(--md-sys-color-on-primary-container)]' : 'bg-[var(--md-sys-color-surface-variant)] text-[var(--md-sys-color-on-surface-variant)]' }} flex items-center justify-center shrink-0 shadow-inner">
                    ...
                    <div class="flex flex-col gap-1 min-w-0 flex-1">
                        <span
                            class="text-sm font-bold text-[var(--md-sys-color-on-surface)] tracking-tight truncate {{ $activeHistoryTab === 'cancelled' ? 'line-through decoration-1' : '' }}"
                ...
                @elseif($activeHistoryTab === 'previous')
                    <div
                        class="w-8 h-8 sm:w-10 sm:h-10 rounded-xl flex items-center justify-center text-[var(--md-sys-color-primary)] bg-[var(--md-sys-color-primary-container)]/30 shrink-0 ml-1">
                ...
                @elseif($activeHistoryTab === 'cancelled')
                    <div class="flex flex-col items-center justify-center ml-1">
                        <span
                            class="text-[9px] font-bold px-2 py-0.5 rounded-md {{ $reservation->status === 'cancelled_admin' ? 'bg-[var(--md-sys-color-error-container)] text-[var(--md-sys-color-on-error-container)]' : 'bg-[var(--md-sys-color-surface-variant)] text-[var(--md-sys-color-on-surface-variant)]' }}">
                            {{ $reservation->status === 'cancelled_admin' ? 'مدیریت' : 'شخصی' }}
                        </span>
                        <span
                            class="material-symbols-rounded text-[18px] sm:text-[20px] text-[var(--md-sys-color-error)] mt-1">block</span>
                    </div>
                @endif
            </div>
        @empty
            <div class="p-8 sm:p-10 text-center flex flex-col items-center">
                <div
                    class="w-16 h-16 rounded-2xl bg-[var(--md-sys-color-surface-container-highest)] text-[var(--md-sys-color-on-surface-variant)] flex items-center justify-center mb-3 opacity-50">
