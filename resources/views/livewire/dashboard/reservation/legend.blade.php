@php
    ...
    $historyRows = [
        ...
        ['icon' => 'event_busy', 'color' => 'error', 'label' => 'لغو شده', 'text' => 'رزروهای لغوشده (توسط شما یا ادمین)؛ نام رزرو به‌صورت خط‌خورده نمایش داده می‌شود و اگر علت لغو ثبت شده باشد، آن علت و نام لغوکننده هم در همان ردیف دیده می‌شود.'],
    ...
    $notes = [
        ...
        'در «تاریخچه من»، با نگه‌داشتن نشانگر روی هر رزرو، گوشهٔ چپ آن یک آیکون زنگ یادآوری ظاهر می‌شود؛ اگر برای آن رزرو یادآوری تنظیم کرده باشید، همیشه با رنگ اصلی برنامه نمایان می‌ماند و با کلیک، همان یادآوری موجود برای ویرایش باز می‌شود.',
    ];
@endphp
...
<div x-data="{ tab: 'statuses' }">
    ...
    <div x-show="tab === 'notes'" x-cloak class="space-y-2">
        ...
        @if(($activeTab ?? null) === 'meeting')
            <div wire:key="reservation-legend-meeting-note" class="flex items-start gap-2 px-1">
        ...
        @endif
        @if(($view ?? null) === 'avantgarde')
            <div wire:key="reservation-legend-grid-note" class="flex items-start gap-2 px-1">
                <span class="material-symbols-rounded text-[15px] mt-0.5 text-[var(--md-sys-color-on-surface-variant)] opacity-70">table_rows</span>
                <p class="text-[11.5px] leading-6 text-[var(--md-sys-color-on-surface-variant)]">در نمای «تاریخچه من»، با نگه‌داشتن نشانگر روی هر ردیف، گوشهٔ چپ ستون آخر سه دکمه ظاهر می‌شود: «پالت» برای رنگ‌آمیزی ردیف، «سنجاق» برای نمایش دائم ردیف در بالای فهرست، و زنگ یادآوری. سنجاق و رنگ فقط در مرورگر خودتان ذخیره می‌شوند (به‌جز خودِ یادآوری که سرور آن را نگه می‌دارد) و محتوای رزروها را تغییر نمی‌دهند. دکمهٔ «مشاهده جزئیات» شما را به نمای رزرو همان منبع می‌برد. سربرگ جدول — فیلتر بازهٔ تاریخ، «نمایش فشرده» و انتخاب ستون‌ها — هم همین‌طور فقط در مرورگر خودتان ذخیره می‌شوند (فیلتر تاریخ نتیجهٔ فهرست را تغییر می‌دهد)؛ ستون آخر همیشه می‌ماند.</p>
            </div>
            <div wire:key="reservation-legend-booking-note" class="flex items-start gap-2 px-1">
                <span class="material-symbols-rounded text-[15px] mt-0.5 text-[var(--md-sys-color-on-surface-variant)] opacity-70">event_available</span>
                <p class="text-[11.5px] leading-6 text-[var(--md-sys-color-on-surface-variant)]">در نمای رزرو، روزهای غیرقابل‌انتخاب تقویم با نگه‌داشتن نشانگر دلیلشان را نشان می‌دهند (روز گذشته، روز غیرمجاز طبق قوانین رزرو — خط‌خورده — یا خارج از بازه). روی ریل زمانی، بخش‌های راه‌راه یعنی همان منبع در آن بازه رزرو دارد؛ اگر بازهٔ شما با آن‌ها همپوشانی شود قرمز و برچسب «تداخل با رزرو فعلی» می‌گیرد — این هشدار فقط دیداری است و تصمیم نهایی با اعتبارسنجی هنگام ثبت است. دستگیره‌ها را می‌توان با ماوس کشید، با کلیک روی ریل به نزدیک‌ترین دستگیره پرش کرد یا با کلیدهای جهت‌داد (هر گام ۳۰ دقیقه) جابه‌جا کرد. همچنین با کلیدهای چپ/راست یا دکمه‌های روی تصویر می‌توانید بدون کلیک، بین منابع موجود دیگر جابه‌جا شوید؛ جزئیات هر منبع در یک ردیفِ زیر تصویر آن نمایش داده می‌شود.</p>
            </div>
        @endif
    </div>
</div>