...
<div
    class="flex-1 min-w-[280px] sm:min-w-[320px] md:min-w-[350px] h-[calc(100vh-180px)] sm:h-[calc(100vh-200px)] md:h-[calc(100vh-220px)] flex flex-col gap-3 md:gap-4 rounded-3xl bg-[var(--md-sys-color-surface-container-low)] p-3 md:p-4 shadow-sm border border-[var(--md-sys-color-outline-variant)]/40 transition-all duration-300"
...
<div class="flex items-center justify-between gap-3 px-2">
    <div class="flex items-center gap-3">
        <!-- Icon Container -->
        <div class="w-10 h-10 rounded-xl bg-[var(--md-sys-color-{{ $config['color'] }}-container)] text-[var(--md-sys-color-on-{{ $config['color'] }}-container)] flex items-center justify-center shadow-sm">
...
<!-- Tasks Container -->
<div class="flex-1 overflow-y-auto overflow-x-hidden space-y-3 px-1 scroll-smooth" style="scrollbar-width: thin; scrollbar-color: color-mix(in srgb, var(--md-sys-color-primary) 30%, transparent) transparent;">
    @forelse($columnTasks as $task)
        @include('livewire.dashboard.taskboard.partials.card', ['task' => $task, 'column' => $column])
    @empty
        <!-- Empty State -->
        <div class="flex flex-col items-center justify-center h-48 text-[var(--md-sys-color-on-surface-variant)]/40 border-2 border-dashed border-[var(--md-sys-color-outline-variant)]/30 rounded-2xl bg-[var(--md-sys-color-surface)]/20">
    ...
    class="h-32 rounded-2xl border-2 border-dashed border-[var(--md-sys-color-primary)] bg-[var(--md-sys-color-primary-container)]/10 flex items-center justify-center gap-2"
    >
