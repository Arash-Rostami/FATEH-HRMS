<x-dashboard.modal.slideover name="create-task-modal" title="وظیفه جدید" icon="add_task">
    <form wire:submit="createTask" class="flex flex-col h-full bg-[var(--md-sys-color-surface)]" dir="rtl">
        <div class="flex-1 overflow-y-auto p-4 md:p-6 space-y-6 custom-scrollbar">
            <div>
                <x-dashboard.form.input label="عنوان وظیفه" name="newTask.title" wire:model="newTask.title" icon="title" placeholder="مثال: بررسی گزارش ماهیانه فروش" required />
            </div>

            <div>
                <x-dashboard.form.textarea label="توضیحات (اختیاری)" name="newTask.description" wire:model="newTask.description" icon="description" placeholder="جزئیات بیشتر درباره این وظیفه..." rows="4" />
            </div>

            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <x-dashboard.form.select label="وضعیت" name="newTask.status" wire:model="newTask.status" icon="pending_actions" required>
                ...
                <div x-data="{
                        ...
                        get selectedUser() {
                            return this.users.find(u => u.id == $wire.get('newTask.assigned_to'));
                        }
                    }" class="relative">
                    ...
                    <div x-show="open" x-transition x-cloak class="absolute z-50 w-full mt-1 bg-[var(--md-sys-color-surface)] border border-[var(--md-sys-color-outline-variant)] rounded-xl shadow-lg overflow-hidden flex flex-col max-h-60">
                        ...
                        <ul class="flex-1 overflow-y-auto py-1 custom-scrollbar">
                            <li @click="$wire.set('newTask.assigned_to', null); open = false; search = ''" class="flex items-center gap-2 px-3 py-2 cursor-pointer hover:bg-[var(--md-sys-color-surface-variant)]/50 transition-colors" :class="!selectedUser ? 'bg-[var(--md-sys-color-primary)]/10 text-[var(--md-sys-color-primary)]' : 'text-[var(--md-sys-color-on-surface)]'">
                            ...
                            <template x-for="user in filteredUsers" :key="user.id">
                                <li @click="$wire.set('newTask.assigned_to', user.id); open = false; search = ''" class="flex items-center gap-2 px-3 py-2 cursor-pointer hover:bg-[var(--md-sys-color-surface-variant)]/50 transition-colors" :class="selectedUser?.id == user.id ? 'bg-[var(--md-sys-color-primary)]/10 text-[var(--md-sys-color-primary)]' : 'text-[var(--md-sys-color-on-surface)]'">
            ...
            <div>
                <x-dashboard.form.input type="datetime-local" label="مهلت انجام (اختیاری)" name="newTask.deadline" wire:model="newTask.deadline" icon="event" />
        ...
        <div class="p-4 border-t border-[var(--md-sys-color-outline-variant)] bg-[var(--md-sys-color-surface)] flex gap-3">
            <x-dashboard.form.button type="submit" variant="primary" icon="save" loading="createTask" class="flex-1">ایجاد وظیفه</x-dashboard.form.button>
            <x-dashboard.form.button type="button" variant="outline" @click="$dispatch('close-modal', 'create-task-modal')">انصراف</x-dashboard.form.button>
        </div>
    </form>
