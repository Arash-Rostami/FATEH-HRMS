<div x-show="tab === 'classification'" class="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-5">
    ...
    <x-ui.forms.select label="واحد سازمانی/دپارتمان" name="form.departmentId" wire:model.live="form.departmentId">
    ...
    <x-ui.forms.select label="واحد (زیرمجموعه)" name="form.unit" wire:model="form.unit">
        <option value="">انتخاب کنید</option>
    ...
    <x-ui.forms.select label="بخش (زیرمجموعه)" name="form.section" wire:model="form.section">
        <option value="">انتخاب کنید</option>
    ...
    <x-ui.forms.select label="جوابگو" name="form.responsibleUserId" wire:model="form.responsibleUserId">
        <option value="">انتخاب کنید</option>
    ...
    <x-ui.forms.input label="پروژه" name="form.project" wire:model="form.project" icon="folder"/>
    <x-ui.forms.input label="طرح" name="form.scheme" wire:model="form.scheme" icon="assignment"/>

    <div class="md:col-span-2">
        <x-ui.forms.select label="همکاران" name="form.collaborators" wire:model="form.collaborators" multiple class="min-h-[100px]">
            @foreach($staffMembers as $staff)
                <option value="{{ $staff['id'] }}">{{ $staff['full_name'] }}</option>
            @endforeach
        </x-ui.forms.select>
...
<div x-show="tab === 'action'" class="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-5">
    ...
    <x-ui.forms.textarea label="حوزه منشاء اقدام" name="form.actionSourceDomain" wire:model="form.actionSourceDomain" icon="hub" rows="2"/>
    <x-ui.forms.textarea label="منشاء اقدام" name="form.actionSource" wire:model="form.actionSource" icon="hub" rows="2"/>

    <x-ui.forms.select label="تعیین تکلیف" name="form.state" wire:model="form.state">
        <option value="">انتخاب کنید</option>
        <option value="extension">تمدید</option>
        <option value="suspension">توقف</option>
        <option value="completion">تکمیل</option>
    </x-ui.forms.select>
