<div class="space-y-5 animate-slideDownFade" dir="rtl">
    <div class="relative overflow-hidden rounded-2xl bg-[var(--md-sys-color-error-container)] text-[var(--md-sys-color-on-error-container)] border border-[var(--md-sys-color-error)]/20 p-5 shadow-sm">
        <div class="absolute -right-4 -top-4 opacity-[0.08] pointer-events-none">
            <span class="material-symbols-rounded text-[100px]">security</span>
        </div>
    ...
    @if($hasAny)
        <div class="relative max-w-sm">
            <x-dashboard.form.input wire:model.live.debounce.300ms="search" label="جستجو در سامانه‌ها..." name="search" icon="search"/>
        </div>

        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            @forelse($credentials as $cred)
                <div class="group relative flex flex-col rounded-2xl border border-[var(--md-sys-color-outline-variant)] bg-[var(--md-sys-color-surface)] overflow-hidden transition-all duration-300 hover:border-[var(--md-sys-color-primary)]/40 hover:shadow-[0_8px_24px_color-mix(in_srgb,var(--md-sys-color-primary)_18%,transparent)] hover:-translate-y-1 shadow-sm">
                    <div class="h-[3px] w-full bg-[var(--md-sys-color-primary)] shadow-[0_0_8px_color-mix(in_srgb,var(--md-sys-color-primary)_40%,transparent)]"></div>
                    ...
                    <div class="p-5 flex flex-col gap-4 relative z-10 h-full">
                        ...
                        <div x-data="{ show: false }">
                            ...
                            <div class="flex items-center justify-between bg-[var(--md-sys-color-surface-variant)]/40 rounded-xl px-3 py-2 border border-[var(--md-sys-color-outline-variant)]/60 hover:border-[var(--md-sys-color-primary)]/40 transition-colors">
                                <div class="flex items-center gap-2 overflow-hidden flex-1">
                                    <button type="button" @click="show = !show" class="flex-shrink-0 text-[var(--md-sys-color-on-surface-variant)] hover:text-[var(--md-sys-color-primary)] transition-colors focus:outline-none">
                        ...
                        @if($cred->link)
                            <div class="mt-auto pt-1">
                                <a href="{{ $cred->link }}" target="_blank" class="flex items-center justify-center gap-2 w-full py-2.5 rounded-xl bg-[var(--md-sys-color-primary)] text-[var(--md-sys-color-on-primary)] text-sm font-bold hover:brightness-105 active:scale-[0.98] transition-all shadow-sm">
            ...
            @empty
                <div class="col-span-full">
                    <div class="flex flex-col items-center justify-center py-16 text-center bg-[var(--md-sys-color-surface)] rounded-2xl border border-dashed border-[var(--md-sys-color-outline-variant)] shadow-sm">
                        ...
                        <button type="button" wire:click="$set('search', '')" class="mt-5 px-5 py-2 rounded-xl bg-[var(--md-sys-color-primary-container)] text-[var(--md-sys-color-on-primary-container)] font-bold text-sm hover:brightness-95 transition-all">
                            پاک کردن جستجو
                        </button>
                    </div>
                </div>
