<div class="space-y-5 animate-[fade-in_0.4s_ease-out]" dir="rtl">
    ...
    <div class="relative overflow-hidden rounded-2xl bg-[var(--md-sys-color-error-container)] text-[var(--md-sys-color-on-error-container)] border border-[var(--md-sys-color-error)]/20 p-5">
        ...
        <div class="relative z-10 flex gap-4 items-start md:items-center">
            ...
            <div>
                ...
                <p class="text-xs opacity-90 leading-relaxed">این داده‌ها را کاملاً محرمانه نگه‌داشته و از اشتراک‌گذاری
                    آن‌ها با سایر همکاران جداً خودداری نمایید.</p>
            </div>
        </div>
    </div>
    ...
    @if($this->hasAnyCredentials)
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            @forelse($this->credentials as $cred)
                <div wire:key="cred-{{ $cred->id }}" data-rf="credentials-{{ $cred->id }}"
                     class="group relative flex flex-col rounded-2xl border border-[var(--md-sys-color-outline-variant)] bg-[var(--md-sys-color-surface)] overflow-hidden transition-all duration-200 hover:border-[var(--md-sys-color-primary)]/40 hover:shadow-md hover:-translate-y-0.5 shadow-sm">
                    ...
                    <div class="p-5 flex flex-col gap-4 relative z-10 h-full">
                        ...
                        <div class="flex items-center gap-3">
                            ...
                            <h3 class="font-bold text-base text-[var(--md-sys-color-on-surface)] truncate"
                                title="{{ $cred->app_name }}">{{ $cred->app_name }}</h3>
                        </div>

                        {{-- Username --}}
                        <div>
                            <div class="text-[10px] uppercase tracking-widest font-bold text-[var(--md-sys-color-on-surface-variant)] mb-1.5">
                                نام کاربری
                            </div>
                        ...
                        <div x-data="{ show: false }">
                            ...
                            <div class="flex items-center justify-between bg-[var(--md-sys-color-surface-variant)]/40 rounded-xl px-3 py-2 border border-[var(--md-sys-color-outline-variant)]/60 hover:border-[var(--md-sys-color-primary)]/40 transition-colors">
                                <div class="flex items-center gap-2 overflow-hidden flex-1">
                                    ...
                                    <span x-show="!show"
                                          class="font-mono text-sm tracking-[0.3em] text-[var(--md-sys-color-on-surface-variant)] pt-1">••••••••</span>
    ...
    @else
        ...
        <div class="flex flex-col items-center justify-center py-20 text-center bg-[var(--md-sys-color-surface)] rounded-2xl border border-dashed border-[var(--md-sys-color-outline-variant)] shadow-sm">
            ...
            <p class="text-sm text-[var(--md-sys-color-on-surface-variant)] max-w-sm leading-relaxed">هیچ حساب کاربری
                برای سامانه‌های سازمانی توسط ادمین برای شما تعریف نشده است.</p>
        </div>
    @endif
