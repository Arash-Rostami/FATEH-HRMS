<div class="relative w-full overflow-hidden rounded-2xl bg-[var(--md-sys-color-surface)] shadow-sm border border-[var(--md-sys-color-outline-variant)]/50">
    <div class="absolute inset-0 pointer-events-none opacity-[0.5]">
        ...
        <div class="absolute inset-0 bg-[linear-gradient(to_right,rgba(0,0,0,0.035)_1px,transparent_1px),linear-gradient(to_bottom,rgba(0,0,0,0.035)_1px,transparent_1px)] [background-size:44px_44px] [mask-image:radial-gradient(circle_at_center,black_45%,transparent_100%)]"></div>
    ...
    <div class="relative p-6 sm:p-8 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6">
        <div class="flex items-center gap-6">
            <div class="relative group">
                <div class="absolute inset-0 bg-gradient-to-tr from-[var(--md-sys-color-primary)] to-[var(--md-sys-color-tertiary)] rounded-2xl opacity-40 group-hover:opacity-60 transition-opacity duration-500"></div>
                ...
                <div class="absolute bottom-0 right-0 w-6 h-6 bg-[var(--md-sys-color-primary)] border-2 border-[var(--md-sys-color-surface)] rounded-full flex items-center justify-center shadow-sm"
                     title="وضعیت: فعال">
            ...
            <div class="flex flex-col gap-1">
                <div class="flex items-center gap-2">
                    ...
                    @if(isset($roleBadge) && $roleBadge)
                    <span class="px-2 py-0.5 rounded-md border border-[var(--md-sys-color-primary)] text-[var(--md-sys-color-primary)] text-[10px] font-bold tracking-wide">
                        {{ $roleBadge }}
                    </span>
                    @endif
                </div>
                ...
                <div class="flex flex-wrap items-center gap-3 mt-1">
                    ...
                    |
                    <span class="inline-flex items-center gap-1.5 text-xs text-[var(--md-sys-color-on-surface-variant)]" title="تاریخ عضویت">
                        <span class="material-symbols-rounded text-[16px] opacity-70">event</span>
                        عضویت:
                        {{ $joinDate }}
                    </span>
                    @endif
                </div>
                @if(isset($bioSnippet) && $bioSnippet)
                <div class="mt-2 text-xs text-[var(--md-sys-color-on-surface-variant)] opacity-90 max-w-md">
                    {{ $bioSnippet }}
                </div>
                @endif
            </div>
        ...
        <div class="w-full sm:w-64 bg-[var(--md-sys-color-primary-container)]/50 rounded-xl p-4 border border-[var(--md-sys-color-outline-variant)]/30">
