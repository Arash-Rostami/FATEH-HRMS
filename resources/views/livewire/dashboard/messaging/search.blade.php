<div x-show="searchMessages" x-transition style="display:none;" class="absolute left-0 right-10 z-40 max-md:right-[-2rem]">
    <div class="rounded-2xl bg-[var(--md-sys-color-surface)] shadow-2xl border border-[color-mix(in_srgb,var(--md-sys-color-outline-variant)_40%,transparent)] overflow-hidden">
        <x-ui.forms.search-field model="messageSearch" id="msg-search-input" debounce="600"
                                 ...
                                 loading-disabled :wire-ignore-self="false"
                                 input-class="w-full h-11 pr-11 pl-10 rounded-2xl text-sm outline-none bg-[color-mix(in_srgb,var(--md-sys-color-surface-variant)_60%,transparent)] text-[var(--md-sys-color-on-surface)] placeholder:text-[color-mix(in_srgb,var(--md-sys-color-on-surface-variant)_70%,transparent)] border-none" />

        @if(strlen(trim($this->messageSearch)))
            <div wire:key="msg-search-results" class="max-h-72 overflow-y-auto contact-scrollbar border-t border-[color-mix(in_srgb,var(--md-sys-color-outline-variant)_30%,transparent)]">
                @forelse($this->messageSearchResults as $r)
                    <button type="button"
                        ...
                        <span class="block text-[12px] leading-relaxed text-[var(--md-sys-color-on-surface)] line-clamp-2" x-html="highlight(@js($r['body']), $wire.messageSearch)"></span>
                ...
                @empty
                    <div wire:key="msg-search-empty" class="px-3 py-5 text-center text-[11px] text-[var(--md-sys-color-on-surface-variant)]">پیامی یافت نشد</div>
                @endforelse
            </div>
        @endif
    </div>
