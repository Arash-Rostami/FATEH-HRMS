...
@if(!empty($cards))
    <div wire:key="tasksheet-highlights" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
        @foreach($cards as $card)
            <div wire:key="tasksheet-highlight-{{ $card['item']['task_id'] }}" class="rounded-2xl border border-[var(--md-sys-color-outline-variant)] bg-[var(--md-sys-color-surface)] shadow-sm p-4 flex flex-col gap-2 animate-slide-up-fade animate-delay-{{ $loop->index * 100 }} hover:border-[var(--md-sys-color-primary)]/40 hover:shadow-[0_8px_24px_color-mix(in_srgb,var(--md-sys-color-primary)_12%,transparent)] transition-[border-color,box-shadow] duration-300">
                ...
                <div class="flex flex-wrap items-center gap-2 text-xs text-[var(--md-sys-color-on-surface-variant)]">
                    @if($card['kind'] === 'hardest_close')
                        <span>{{ convertToPersian(number_format($card['item']['cycle_time_days'], 1)) }} روز</span>
                    ...
                    @elseif($card['kind'] === 'most_collaborated')
                        <span>{{ convertToPersian($card['item']['comments_count']) }} نظر همکاران</span>
                    @endif
                </div>
            </div>
        @endforeach
