...
**Per-user derived counts under a Tier V key (added 2026-09-14):** `DMS::pendingCounts($userId)` (900s; `[needsSign, needsRead]` — feeds the navbar widget and `DmsBadge`/`hasPendingFor`), `DMS` filter-chip groups (`filter_groups:{tab}:{userId}`, 900s, `Dms/Main.php`), `EnergyTest::company_averages:{userId}` (900s, `Energy/Chart.php` — per-user because it excludes the viewer), `Feed`/`Post` `total_count` (900s), `Post` `non_pinned_count` (900s, `Tab\Posts::hasMorePosts()` load-more gate), and `DatabaseNotification` `unread_menu_groups:{userId}` (900s, `UnreadNotifications.php` — the per-render JSON group-by). All use `ModelCacheVersion::remember(ThatModel::class, …)` so the model's own writes auto-invalidate.

**Cross-model drift rule — the derived value can change WITHOUT the versioned model being written.** `pendingCounts` is versioned on `DMS`, but a user confirming a read writes the `Read` model, not `DMS` — the version never bumps, so the navbar count would sit stale for the full TTL right after the action that should change it. The rule: when the underlying state change is a write to a DIFFERENT model, the action that performs it adds one targeted `Cache::forget(ModelCacheVersion::key(VersionedModel::class, 'suffix:{userId}'))` (see `ConfirmReadAction`). Only the acting user's key is forgotten — a flush-style `bump` would orphan every user's cache for one user's action.
...
**Tier L models (current):** `Read` (written on every feed view by every user), `Reaction`, `Comment`, `Reply`, `Message`, `ChannelMessage`, `ChannelMember` (membership/unread pivot), `SkillUser`, `EventShare`, `TaskDetail`, `DMS` thread mutation, `Task` board (`TaskBoard\Main` uses plain `#[Computed]`, no cross-request cache — a status flip re-queries fresh next render), `Reservation` calendar (`Reservation\Main` uses plain `#[Computed]` — a booking re-queries fresh next render), `Ticket` (list is live; reply counts are live via the `Reply` high-churn stream), `Poll` (results are a live `loadMissing('polls')` relation query — 300s stale vote counts is bad UX), `Channel` list (membership-scoped, live), the `FAQ` / `Feed` **lists** themselves (filtered/paginated/user-scoped → live; only their **categories** are Tier V), and `Reminder` (no `cached*()` helper at all — every scope in `app/Livewire/Dashboard/Reminder/Widget.php` (`reminders()`, `suggestedTitles()`, `dueCount()`) hits the DB directly through plain `#[Computed]`; a per-user reminder list is small and must reflect a just-created/snoozed/completed row on the very next render, so there is nothing here worth a cache layer — the first model deliberately classified Tier L with zero cache code, not merely "not yet cached").
