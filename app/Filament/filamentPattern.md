...
**Same mandatory-fan-out rule applies to dashboard stats.** Every admin Filament resource with meaningful aggregate data (counts, status breakdowns) also gets a tab in `ModuleAnalyticsWidget` (`app/Filament/Widgets/ModuleAnalyticsWidget.php`) — see "Widget sorting convention" above and the full checklist in `widgetsPattern.md`. Skip only for a pure pivot/junction resource already folded into its parent's tab.
...
1. **Column-based** — set `protected static ?string $recordTitleAttribute = '<column>';` when one column identifies the record (`UserResource` → `'name'`, `ThsResource` → `'request_subject'`, …). Filament reads it natively; no method needed.
2. **Composite / enum / accessor** — override `getRecordTitle(?Model $record): ?string` when the identity is a combination, a non-column value, or an HTML-stored column (`ReservationPolicyResource` → `ResourceType::tryFrom($record->resource_type)?->getLabel()`, `ProfileResource` → `$record->user?->name ?? '#'.$record->personnel_id`, `ResourceResource` → `$record->labeled_name`, `ContactResource` → `Str::limit($record->body, 60)` (Textarea, plain), `FeedResource` → `Str::limit(strip_tags($record->content), 60)` and `PostResource` → `strip_tags($record->title)` and `FAQResource` → `strip_tags($record->question)` — those three columns are `RichEditor` storing HTML, so bare `recordTitleAttribute` / un-stripped `Str::limit` would surface raw `<p>/<strong>` tags in the View/Edit heading + breadcrumb). Always guard `if (! $record) return null;` first — the `?Model` signature permits null (Filament may call before a record is loaded).
...
- **Never use the column lever (`$recordTitleAttribute`) when the column is a `RichEditor`** — Filament returns the raw HTML and the heading/breadcrumb render literal `<p>/<strong>` tags. Use the method lever and `strip_tags()`; mirror `getGlobalSearchResultTitle()`, which already strips for the same reason. Any `getRecordTitle` returning `RichEditor` content must `strip_tags()` *before* `Str::limit()`/display (stripping first also prevents `Str::limit` from cutting mid-tag and emitting broken markup).
...
**A resource-local enum's backing string values are the single source of truth wherever that domain concept has an ordinal rank.** `ProfileResource/Enums/Position.php` backs `HasProfileHierarchy::RANKS` (`app/Models/Concerns/HasProfileHierarchy.php`) — every enum case's value is a `RANKS` key (lower number = higher rank). Adding a case means updating both the enum's match arms (label/color/icon) and `RANKS` in lockstep; every consumer (admin form/table/infolist/export, `rank()`, `isDeptHead()`, `highestRankingInDepartments()`, and the user-panel Status board's `ORDER BY FIELD(...)` sort) reads from one of these two places, so nothing else needs touching. Never hard-code the position list a third time elsewhere.
...
## Admin notifications — `Notification::make()` (ephemeral toast vs persistent bell)

Filament ships its own notification primitive; the admin panel uses it directly, the user panel does
**not** (it has a separate lightweight toast — see the contrast at the end of this section).

- **Ephemeral admin toast** — `Notification::make()->title(…)->success()/danger()/warning()/info()->send()`
  renders Filament's auto-dismissing toast at the top of the panel. No DB row, gone in seconds. The
  canonical in-resource shape is `FilamentActions`' assign/unassign/bulk-unassign actions
  (`Notification::make()->success()->send()` inside the `->action(…)` closure, after the handler runs).
  Use this for "the write succeeded / failed" feedback inside an admin page.
- **Persistent bell entry** — `->sendToDatabase($user)` writes a row to Filament's `notifications`
  table (stays until read); `->persistent()` marks it non-auto-dismiss. `->actions([Action::make(…)
  ->dispatch(event, data)->close()])` attaches a button that dispatches a Livewire event on click.
  `App\Traits\InteractsWithNotifications::notifyWithAction($title, $body, $eventName, $eventData)`
  is the one-liner wrapper: `Notification::make()->success()->actions([Action::make('view')->button()
  ->dispatch(…)->close()])->sendToDatabase(auth()->user())`. Mix into any Filament component that
  needs to push a dismissible, action-bearing bell entry from an admin action.
- **The user-panel bell** is the same `notifications` table rendered by
  `App\Livewire\Dashboard\UnreadNotifications` (extends Filament's `DatabaseNotifications`), which also
  folds >4-per-key rows into aggregate rows (`agg:` synthetic ids, DMS sign/read split). The rows it
  lists are written by `App\Services\Menu\NudgeService` (`->warning()->persistent()->actions([view,
  markAsRead])->getDatabaseMessage()`), the bell/nudge layer documented in `statePattern.md`.

**Contrast — user-panel ephemeral toast is NOT `Notification::make()`.** The non-Filament user panel
has a separate, hand-built auto-dismissing toast (`<x-ui.modals.toast/>`, no DB, `window` CustomEvent
driven), documented in full at `app/Services/Menu/statePattern.md` ("Ephemeral toast"). Do not call
`Notification::make()->send()` from user-panel Livewire components — it would render into a Filament
mount that does not exist there. Admin → `Notification::make()`; user-panel → `x-ui.modals.toast`.
...
master catalog (all 17 badges, grouped into themed tabs) in the actual Profile module
...
## `ProjectResource` (cross-reference, not a new mechanism)

`ProjectResource` follows this doc's standard shape verbatim (Schema classes, `FilamentAdminGuide`/`FilamentActions`/`FilamentFilters`/`AuthorizesByPermission` traits, a `TasksRelationManager`) — nothing resource-specific to add here. Its **domain-specific** conventions (the `member_ids`/`departments` audience-JSON UI, `scopeVisibleTo()`, the shared `ReportingService` reused verbatim by its table/infolist, the `channel_id` provisioning link) are documented in full at `app/Livewire/Dashboard/Project/projectPattern.md`, since that module owns the domain model this resource administers. Read that file for anything beyond generic Filament wiring.
...
`#[Computed(seconds: 300, cache: true)]` caches each analytics method for 5 minutes under `lw_computed.{component}.{prop}` — **and this only works because every method is consumed as a bare no-argument property** (`$this->usersData`, `$this->adsData`, … in `getActiveStatsCount` / `statsSchema`), which routes through `BaseComputed::handleMagicGet`. A method *called directly* (`$this->getModuleAData($dept)`) bypasses that path so the attribute is inert there — see rule 25 and the `OperationalChart` / `StructuralChart` / `HrAnalyticsService` `rememberGlobal` pattern.

**MANDATORY — every new admin Resource extends this widget.** Full checklist (data method + `getAllTabs()`/`getActiveStatsCount()`/`statsSchema()` registration + magic-property-only consumption + test coverage) lives in `app/Filament/Widgets/widgetsPattern.md` under "`ModuleAnalyticsWidget` — tabbed stats" — treat it as the same never-skip fan-out as `FilamentAdminGuide`/legend (§3.4).
...
**61. Cardinal rule — admin panel icons are Heroicons, user panel icons are Material Symbols, and a value picked in one system is NEVER reused raw in the other.** This project enforces (see `project_icon_convention_admin_vs_user` / the admin-vs-user icon-convention notes elsewhere): Filament admin UI (`->icon()` on table columns, infolist entries, actions, `<x-icon>`) uses Heroicon identifiers (`heroicon-o-*`); the user panel renders icons as literal ligature text inside `<span class="material-symbols-rounded">{value}</span>`. **Any field where an admin picks ONE icon that must be shown in both places needs an explicit closed-enum translation step — never a shared raw string.** Reference implementation: `App\Enums\ResourceTypeIcon` carries both a Heroicon `value` (admin) and a `getMaterialIcon()` match (user panel); `ResourceResource`'s `TypeProvisioner` derives and stores `display_material_icon => ResourceTypeIcon::from($data['display_icon'])->getMaterialIcon()` at write time — the two representations never drift because one is mechanically derived from the other via a `match()` with no `default` arm (a new case added to the enum without a corresponding `getMaterialIcon()` arm throws `UnhandledMatchError` immediately, not silently — fail loud, not silently garbled). **The bug this rule exists to prevent, found live in production:** `Department`'s `ticket_options.icon` repeater field used this exact `ResourceTypeIcon` picker correctly for the admin-side Heroicon path (`HasTicketOptions::getCustomHeroiconForArea()`), but its Material Symbols read path (`getCustomMaterialIconForArea()`) reused the *same raw stored Heroicon string* directly, printing literal text like `heroicon-o-computer-desktop` inside a `material-symbols-rounded` span in the live user-panel ticket UI (`ths/create.blade.php`, `ths/ticket-table.blade.php`). Fixed by routing the material-icon read through `ResourceTypeIcon::tryFrom($stored)?->getMaterialIcon()` before falling back to the area's default lookup — never trust that a stored "icon" column is already in the right icon system for wherever you're about to render it. Same principle applies to any hand-written array pairing an icon string with an enum-backed status/type WITHOUT deriving it from that enum's own method (see `DocumentStatus::getMaterialIcon()`, added specifically so `DMS::getStatusIcon()` derives its glyph from the enum instead of maintaining a second parallel array that could silently drift when a new `DocumentStatus` case is added). When building a NEW field where an admin picks an icon for user-panel-only rendering (no admin-side Heroicon use at all), skip the dual-system translation entirely and use a **Material-Symbols-native closed enum** instead (see `App\Enums\LinkIcon`, `App\Enums\SkillIcon`) — don't force a Heroicon roundtrip where nothing ever consumes the Heroicon half.
...
**Revised 2026-08-26**: `ModuleAnalyticsWidget` moved from `6` to `-1` (right under the `ManagePreferencesWidget` settings accordion, above the module guide) so the stats are visible near the top of the dashboard instead of below the HR/operations chart blocks; `ModulesGuideWidget` shifted from `-1` to `0` to keep it directly after. Current block: `AccountWidget` `-3`, `ManagePreferencesWidget` `-2`, `ModuleAnalyticsWidget` `-1`, `ModulesGuideWidget` `0`, HR block `1`–`4`, `UnmetSkillDemandTable` `5`, `OperationalChart` `7`, `StructuralChart` `8` (gap at `6` is harmless).
...
private static function logoHtml(bool $dark): HtmlString
{
    ...
    return new HtmlString(sprintf(
        ...
        e(asset(tenantLogo($dark, 'admin', $auth))), e(config('app.name_en')), e(config('app.name_en'))
    ));
}
```

`darkModeBrandLogo()` renders both images; Filament's CSS (`fi-logo-light`/`fi-logo-dark`, plain `dark:` Tailwind variants) toggles visibility off the `.dark` class on `<html>` — no JS needed beyond what's already wired by the app's own theme sync. Works even with `->darkMode(false)` since that flag doesn't gate the CSS. The per-tenant reverse-logo behavior (`admin_reverse_logo` in `config/tenants.php`), its auth-page-only counterpart `admin_auth_reverse_logo`, and the `admin_use_company_logo` short-circuit are all resolved inside `tenantLogo()` itself — `$auth` picks which of the two flags governs the swap, fully independent of each other. Logo paths never hardcoded — see `config/tenantPattern.md` for the full `tenantLogo()` contract.
...
- `nav_dock` (default **off**) and `topbar_pinned` (default **on**) — pure CSS/JS layout toggles with no Filament Panel equivalent at all. Applied via a class added to `<html>` at `PanelsRenderHook::HEAD_START` in `FilamentServiceProvider::registerThemeBootstrap()` (`nav-dock-bottom`, `topbar-pinned`), styled in `resources/css/core/filament.css` under `@media (min-width: 1024px)` blocks (desktop-only):
  ...
  - Dock items get hoverable titles the same way BMS-CM's does: Filament's own sidebar-item Blade (`vendor/filament/filament/resources/views/components/sidebar/item.blade.php`) already ships an `x-tooltip` binding driven by `$store.sidebar.isOpen`, but only renders it when `isSidebarCollapsibleOnDesktop()` is true — so `FilamentPanelCustomizer::apply()` sets `->sidebarCollapsibleOnDesktop(fn() => self::pref('sidebar_collapsible', false) || self::pref('nav_dock', false))`, making dock mode always collapsible-on-desktop regardless of the (mutually-exclusive, disabled-in-UI) `sidebar_collapsible` toggle's own saved value. `resources/js/core/nav-dock.js` then closes `Alpine.store('sidebar')` whenever `.nav-dock-bottom` is present (activating Filament's built-in tooltip) and overrides the resulting tippy instances' `placement` to `'top'` on `.fi-sidebar-item-btn`/`.fi-sidebar-group-dropdown-trigger-btn` (Filament's own effect always computes `right`/`left`, which is wrong once the sidebar is a horizontal strip pinned to the bottom edge). It binds on `alpine:initialized` (not `alpine:init`) because Filament registers `Alpine.store('sidebar')` inside its own `alpine:init` listener — reading the store from a sibling `alpine:init` listener race-depends on script-tag order, `alpine:initialized` fires only once every listener, including Filament's, has already run. Deliberately does **not** call `.open()` when dock is off, unlike BMS-CM's version — an unconditional `open()` would fight a real `sidebar_collapsible` user's own persisted collapse state on every non-dock page load, which BMS-CM never has to worry about since it hardcodes `sidebarCollapsibleOnDesktop()` unconditionally true instead of gating it by preference. **Known residual edge case:** `Alpine.store('sidebar').isOpen` is `Alpine.$persist`-backed (localStorage), so a user who has both `nav_dock` and `sidebar_collapsible` enabled at once can find the classic sidebar still collapsed for one page load after switching dock off (dock's `.close()` set `isOpen=false`; nothing restores it since we never call `.open()`) — self-corrects on the next manual toggle, not worth code for.
  - `topbar_pinned` off makes `.fi-topbar-ctn` a fixed, mostly-offscreen strip that slides fully into view on `:hover`/`:focus-within` (pure CSS transform + transition, no JS for the show/hide itself). One JS file, `resources/js/core/topbar-autohide.js` (registered as both a Vite `input` entry in `vite.config.js` and a `SCRIPTS_BEFORE` render hook in `registerCoreScripts()`), briefly adds `topbar-force-hidden` after `livewire:navigated` so the topbar doesn't flash open during an SPA page transition. That handler must bail out early when `.topbar-pinned` is present — the `html.topbar-force-hidden .fi-topbar-ctn` CSS rule isn't scoped to `:not(.topbar-pinned)` (matching BMS-CM's own unscoped rule, which is only safe there because its JS carries the same guard), so unconditionally adding `topbar-force-hidden` on every SPA nav made the *pinned* (always-visible) topbar visibly slide up and back down on every page transition even though auto-hide was off — the reported "glitchy, needs toggling a few times" symptom. **Label trap (fixed 2026-09-11):** the toggle's own label used to read "اتو هاید نوار بالا" (auto-hide the topbar) while `true`/on is actually the *pinned* (always-visible, auto-hide OFF) state — turning the toggle "on" did the opposite of what its label promised. Relabeled to "ثابت نگه‌داشتن نوار بالا" (keep the topbar pinned) with distinct on/off icons; the underlying field name, default (`true`), and stored semantics are unchanged, only the label/icon now match reality.
  - Because `nav_dock` replaces the sidebar and `top_nav` moves the sidebar's contents to the top, the two are mutually exclusive **in both directions**: `nav_dock` carries `->disabled(fn (Get $get) => (bool) $get('top_nav'))` and `top_nav` carries `->disabled(fn (Get $get) => (bool) $get('nav_dock'))` (fixed 2026-09-11 — previously only the `nav_dock`→`top_nav` direction was guarded, so a user could turn `top_nav` on first and then also turn `nav_dock` on, leaving both true with `nav_dock`'s CSS silently targeting a `.fi-sidebar` that top-navigation mode no longer renders). `sidebar_collapsible` also carries `->disabled(fn (Get $get) => (bool) $get('nav_dock'))`. `sidebar_fully_collapsible` is disabled by **either** `nav_dock` **or** `sidebar_collapsible` (`->disabled(fn (Get $get) => (bool) $get('nav_dock') || (bool) $get('sidebar_collapsible'))`, fixed 2026-09-11) — Filament's own `components/layout/index.blade.php` uses an `@if($isSidebarCollapsibleOnDesktop) … @elseif($isSidebarFullyCollapsibleOnDesktop) …` chain, so when both are on, `sidebar_collapsible` silently wins and `sidebar_fully_collapsible` has zero visible effect; leaving it enabled let a user toggle a setting that visibly did nothing. Likewise `topbar_pinned` and `user_menu_topbar` only make sense when the topbar is actually shown: both carry `->disabled(fn (Get $get) => !$get('topbar'))` (the `user_menu_topbar` guard added 2026-09-11 — without it, a user could force `UserMenuPosition::Topbar` into a panel with no topbar at all, per `FilamentPanelCustomizer::apply()` below, stranding themselves with no visible profile/logout menu anywhere; `FilamentPanelCustomizer::apply()` also carries a defense-in-depth fallback — `(self::pref('user_menu_topbar', false) && self::pref('topbar', true)) ? UserMenuPosition::Topbar : UserMenuPosition::Sidebar` — so even a pre-existing stale/conflicting stored combo self-corrects at render time, not just in the form UI). Every toggle a `disabled()` closure reads (`nav_dock`, `top_nav`, `sidebar_collapsible`, `topbar`) must itself be `->reactive()` for the dependent field's `disabled()` state to update live while the Preferences form is open, without a page reload.
  - **Persistence trap (fixed 2026-09-11):** `FilamentPreferences::save()` used to do `$extra['preferences'] = $this->form->getState();` — a full overwrite. Filament's `->disabled()` also makes a field non-dehydrated (it is silently absent from `getState()`, not merely read-only), so saving the form *while* a dependent toggle was disabled (e.g. `topbar_pinned` while `topbar` was off) silently deleted that toggle's stored value — it would revert to its field default the next time its parent toggle was switched back, with no edit to that toggle itself. `save()` now merges instead of overwriting: `$extra['preferences'] = array_merge($extra['preferences'] ?? [], $this->form->getState())` — an actively-toggled field (even set to `false`) always appears in `getState()` and correctly overrides the merge, so only genuinely-disabled/hidden fields fall back to their previously-stored value instead of vanishing.
...
Strips HTML tags, html-decodes entities (incl. `&nbsp;` → space), normalizes U+00A0/U+200B/U+FEFF to spaces, collapses whitespace, trims. No truncation. Use for full-length title surfaces — `getRecordTitle()` / `getGlobalSearchResultTitle()` — instead of bare `strip_tags()`, which leaves `&nbsp;` and other entities as literal text. Both `stripHtml` and `superClean` preserve emoji (incl. ZWJ sequences — U+200D is not in the normalize list); none of strip_tags / html_entity_decode / `\s` / the U+00A0 replace touch emoji bytes.

### Infolist `TextEntry` for multiline free-text → `white-space: pre-wrap`

Any Infolist `TextEntry` that displays a **full** free-text column (description, notes, body, comments, actions, response, about-me, accessibility, address, interests, revision, etc. — anything a user/admin types with possible newlines/paragraphs) MUST carry `->extraAttributes(['style' => 'white-space: pre-wrap;'])`. Filament's `TextEntry` renders the value with default `white-space: normal`, so `\n` collapses to a space and a multi-paragraph value shows as one run-on line — the admin sees a flattened blob of what the user typed with real line breaks. If the entry already has `->extraAttributes(['style' => '…'])` (e.g. `EventInfolistPresenter::description()` had `margin-bottom:15px`), MERGE `white-space: pre-wrap;` into the SAME style string (`'white-space: pre-wrap; margin-bottom:15px'`) — never add a second `->extraAttributes()` call, the second silently overwrites the first. For a RichEditor-HTML column shown as prose, keep `->prose()` AND add the pre-wrap `extraAttributes` (prose alone doesn't pre-wrap plain-text-authored rows of a dual-authored column — `SuggestionInfolistPresenter::description()` is dual-authored: admin RichEditor OR user plain textarea, so it needs both). For a plain-text column that needs `<br>` rather than CSS pre-wrap, the reference pattern is `ReleaseRequestInfolistPresenter::body()`: `->html()->formatStateUsing(fn(?string $state) => $state === null ? '' : nl2br(e($state)))` (escape + nl2br, XSS-safe). Table `TextColumn` list views truncate (`->limit()`) so pre-wrap is unnecessary there — the rule is for the Infolist VIEW page only. Audit (2026-08-27) added pre-wrap to: Task (description, actionSource, actionSourceDomain, activity reply body), Profile (about_me value, detail display_value, accessibility, address, interests), Resource notes, Event description (merged), Gallery description, Skill description, Department description, Credential note, Link (url_description, image_description), Suggestion (comments, description, referralActions), ReviewsRelationManager (comments, actions), ReleaseRequest response, Dms revision, Contact body, Channel description, Ad (certificate/experience/skill), Ths (actionResult, additionalNotes, description). User-panel mirror of the same rule (blade `whitespace-pre-wrap` + `renderComment()`) is in `livewirePattern.md` "Rendering text/HTML in Blade".
...
**25. Heavy widget analytics must cache — but `#[Computed(cache: true)]` only caches magic-get properties, not direct calls.** `#[Computed(seconds: 300, cache: true)]` engages ONLY through magic-property access (`$this->usersData` → `BaseComputed::handleMagicGet` → `Cache::remember` under `lw_computed.{component}.{prop}`); a method called directly as `$this->getModuleAData($dept)` bypasses that path entirely, so the attribute is dead decoration and the body re-runs on every render. Two correct shapes: (a) the `ModuleAnalyticsWidget` pattern — no-arg `{domain}Data()` methods consumed as bare `$this->usersData` properties, where `#[Computed(seconds: 300, cache: true)]` genuinely caches; (b) the `StructuralChart` / `OperationalChart` / `HrAnalyticsService` pattern — `ModelCacheVersion::rememberGlobal("{$ns}:{$key}", fn () => […])` with the argument baked into the key (300s, config-driven via `app.cache_ttl`). Never put `#[Computed(cache: true)]` on a method called directly with an argument — see `app/Models/modelPattern.md` (V/T/L tiers) and the local guard test.
...
**69. Type-aware JSON metadata fields: fixed keys stay dedicated Filament form fields (never a locked Repeater), custom/cosmetic keys go in a namespaced `metadata.custom` `KeyValue`.** `ResourceResource` (seat/spot/meeting/car, plus any admin-added type — see `livewirePattern.md`'s "Dynamic resource types" section) is the reference: `floor`/`unit`/`extension`/`card`/`capacity` are each a dedicated `ResourceFormPresenter` method gated by `type` (legacy types only, never shown for a new admin-added type) — this is what makes the key "immutable in definition, mutable in value" for free, since there is no UI to rename what column of the JSON a hardcoded form field targets. Exception: `available_days`/`time_slots.{start,end}` are **not** type-gated — they back the generic, type-agnostic `Validators/ResourceSchedule` booking rule, so they're visible/dehydrated for every type including new ones, not just the historically meeting/car pair. Admin-appendable cosmetic attributes go in a **separate namespaced sub-key** `metadata.custom` via `KeyValue::make('metadata.custom')` (mirrors `UserFormPresenter::extra()`), never flattened into the top level — flat storage would let an admin type a custom key named e.g. `floor` and silently corrupt real data; namespacing rules that out structurally. `Resource::formattedMetadata()`'s generic per-key chip loop must explicitly `continue` past `custom` (flatten it separately) and past any field with its own prominent rendering (e.g. `notes`, rendered as a highlighted line above the chip strip in `image.blade.php`, not as a chip) — every other key falls through to the generic icon/label fallback automatically, so a new fixed field only needs a `$config[...]` entry to get a real icon, not new Blade.

**70. Extra logic that doesn't belong directly in a Presenter (multi-field orchestration, side-effecting writes, non-trivial derivations) goes in a dedicated static-method class under `Schemas/Helper/`, never inline in the Presenter method or the Page's `mutateFormDataBeforeCreate`/`Save`.** Reference: `OnboardingResource\Schemas\Helper\ExtrasTemplate` (`add`/`hint`/`options`/`sync`, consumed from `OnboardingFormPresenter` via `ExtrasTemplate::add($state, $set, $get)` etc.) and `ResourceResource\Schemas\Helper\TypeProvisioner` (`provision(array $data): string` — slugifies a new type's label, resolves same-label idempotent reuse vs. different-label collision suffixing, bulk-inserts the full `reservation_policies` row set, busts `ResourceType`'s cache; consumed from `ResourceFormPresenter::type()` via `->createOptionUsing(fn(array $data) => TypeProvisioner::provision($data))`). Namespace mirrors the owning resource: `App\Filament\Resources\{Resource}\Schemas\Helper\{HelperName}`. The dividing line: a Presenter method builds and returns ONE Filament component (`Select`/`TextInput`/etc.) — if the logic a closure needs is more than a few lines of the component's own visibility/formatting/validation, or it performs a write/side-effect (a DB insert, a cache bust, multi-step business logic), it's a Helper's job, referenced from the Presenter closure by a single static call, not written inline. Keeps the Presenter file readable as a flat list of "here's each field" and keeps the actual logic independently testable (a Helper's public statics are called directly in tests, no `ReflectionMethod` needed, unlike a Presenter's private methods).

**71. Icon `Select` fields render the actual glyph AND a Persian text label — never the icon's raw string name, never English — via `App\Traits\FilamentIconOptions` + `allowHtml()` + `getOptionLabelUsing()`.** Both icon-picker consumers (`ResourceFormPresenter::type()`'s `display_icon` field and `DepartmentFormPresenter::ticketOptions()`'s `icon` field) now share the exact same curated set — `App\Enums\ResourceTypeIcon`'s 14 cases, each with a real Persian `getLabel()` — via the trait's `curatedIconOptions()` (for `->options()`) and `curatedIconOptionLabel(string $value)` (for `->getOptionLabelUsing()`), both rendering `<div class="flex items-center gap-2">{svg}<span>{label}</span></div>`. This replaced Department's original design of scanning the entire `vendor/blade-ui-kit/blade-heroicons/resources/svg/*.svg` folder (hundreds of options, `Cache::remember` needed, English filename-derived labels only) — a real behavior change (fewer selectable icons), made deliberately for cross-module consistency once the user asked for "Farsi like resource" rather than mismatched English/Persian pickers. Backward compatibility for already-stored off-catalog icon values (departments that picked something outside the new curated 14 before this change) is preserved: `curatedIconOptionLabel()` falls back to rendering the raw heroicon file with a title-cased English label (the trait's `renderCuratedIconOption()` doesn't require the value to be in the curated enum) so an existing record's selected chip still displays correctly, it just isn't offered as a choice for *new* selections. **Gotcha:** `getAllIcons()`'s `Cache::remember('department_all_icons', 86400, ...)` result does NOT auto-invalidate on a code deploy — a stale 24h-cached value from before this kind of icon-rendering change will keep serving the old HTML shape until it expires or `Cache::forget('department_all_icons')` is run manually. Since the curated approach no longer needs that cache at all (14 static cases, cheap to compute inline), this class of staleness is now moot for the icon fields specifically, but the same gotcha applies to any other `Cache::remember`-wrapped computed-from-code (not from DB) value in this codebase. Wiring: `->options(fn() => ...)` returns `[value => renderedHtml]`, `->allowHtml()` lets Filament render that HTML in the dropdown, `->getOptionLabelUsing(fn(string $value) => ...)` re-renders the same HTML for the already-selected chip (Filament doesn't reuse the `options()` closure's output for that).

**72. Self-referential "parent" select on a string-code-keyed model (never a numeric-PK pattern): reactive `Select` excludes self via `Get $get('code')`, a shared `Model::wouldCreateCycle(?string $code, ?string $parent): bool` static rejects self-reference/cycles by walking the model's own `getCachedModels()` cache (zero extra query, always fresh — the cache is bumped on every save via `ModelCacheServiceProvider`), and the SAME cycle check is duplicated as a model-level `static::saving()` guard so it can't be bypassed by tinker/seeders/imports, not just the form.** Reference: `DepartmentFormPresenter::subordinateTo()` (`Department::getCachedOptions()->except($get('code'))`, `rules(fn (Get $get) => [new NoCyclicDepartmentHierarchy($get('code'))])`) + `Department::wouldCreateCycle()`/`booted()`. This differs from `Reservation`'s or `Comment`'s self-referential `parent_id` (numeric PK, no code-exclusion/cycle machinery needed at that scale) — do not reuse either as a template for a code-PK model. A sibling reactive field pair enforcing "if the parent is set, a dependent field can't hold its 'top-level' value" (here: `subordinate_to` set ⇒ `level` can't be `0`) follows the existing `Get`/`Set`-reactive pattern (rule 22/Task's department→unit→section cascade), with the invariant auto-corrected (not just rejected) both in the form (`Select::make('level')->options(fn (Get $get) => ...)` narrows the option list) and in the model's `saving()` hook (defense-in-depth for the same reason as the cycle guard).

**Before naming a new metadata JSON key, check the live data, not just the code.** Old/orphaned keys can exist in production rows that the current code never reads (this project once shipped `unit`/`card`/`availability` on `resources.metadata` with zero corresponding form fields) — inventing a plausible-sounding new key name (`card_number`) instead of matching what's already stored (`card`) silently splits one field into two: the new admin input reads/writes the new key while all the real historical data sits under the old one, invisible to the new field. Run a `SELECT`/raw SQL export of the actual table before adding a new JSON sub-field to an existing column.

**Gotcha, CONFIRMED IN REAL PRODUCTION BROWSER USE (not just tests) — never use `Get $get('...')` inside a `->visible()`/`->dehydrated()`/`->helperText()` closure on a component whose OWN statePath is itself a dot-notation JSON path (e.g. `TextInput::make('metadata.extension')`); use `fn ($livewire) => $livewire->data['type'] ?? null` instead.** On `ResourceResource`'s edit form, every `metadata.*` field gated by `Get $get('type')` (`floor`, `unit`, `extension`, `capacity`, `available_days`, `time_slots.*`) rendered as an inert, content-less `<div class="fi-hidden">` wrapper — no label, no input, nothing — for EVERY existing record, confirmed by the client viewing a real meeting resource's edit page (only the two ungated fields, `notes`/`custom`, ever appeared) and independently reproduced via `Livewire::test(EditResource::class, [...])->html()` (`wire:partial="schema-component::form.metadata.extension"` wrapper present, but the `x-data="filamentSchemaComponent(...)"` that actually renders the field's content was missing — `Component.php` only emits it `if ($isVisible)`). This is NOT the `->visible()` static-cache staleness that only appears across `Livewire::test()`'s chained `fillForm()` calls (disabling that cache via `Component::disableVisibilityCache()` did **not** fix it) — it is `Get`'s own relative-path resolution failing for `type` when called from inside a component nested under the unrelated `metadata.` prefix, on a completely fresh single-request page load, with `type` correctly set on the record from the start. Swapping every `Get $get('type')` for `$livewire->data['type'] ?? null` (Filament auto-injects `$livewire`, the underlying Livewire component instance, into any schema closure) fixed rendering immediately, confirmed both by the client's live browser and by the same `Livewire::test()->html()` check. Applies to **any** closure on a `metadata.*`/similarly-dot-nested field that needs to read a sibling top-level field — not just `->visible()`.

**Gotcha — Filament dehydrates a "blank" field as an explicit `null` (or `[]` for a `CheckboxList`/`KeyValue`), never as a fully absent key — this silently breaks any downstream code that distinguishes "key absent" from "key present but empty."** `ResourceFormPresenter::timeSlotStart()/timeSlotEnd()` left blank persist as `metadata.time_slots = {"start":null,"end":null}`, not an absent `time_slots` key; an unchecked `availableDays()` persists as `metadata.available_days = []`, not absent. `Resource::scopeAvailable()`'s narrowing (`whereNull('metadata->available_days')` / `whereNull('metadata->time_slots->start')`) only treats a **genuinely absent** key as "no restriction" — `JSON_EXTRACT` of an explicit JSON `null` is NOT SQL `NULL`, and `JSON_CONTAINS` on `[]` is always false — so saving a meeting/car resource through the admin form with the schedule fields left blank (the expected default) silently made it match **zero** days/hours, i.e. permanently unbookable, the moment it was first saved. Fixed via `ResourceFormPresenter::normalizeMetadata(array $data): array`, called from both `CreateResource::mutateFormDataBeforeCreate()` and `EditResource::mutateFormDataBeforeSave()` — strips `time_slots` to fully absent when either half is blank, strips `available_days` to fully absent when empty, restoring "doesn't define these keys ⇒ no restriction" as an actual invariant rather than an assumption. **Rule: any optional JSON sub-field that a query/validator treats as "absent ⇒ unrestricted" needs an explicit strip-when-blank step in a `mutateFormDataBefore{Create,Save}` hook — Filament will never do this for you.**

**73. Every exception message thrown from a model lifecycle hook or a write-path Action class must go through `__()` — never a raw string — because it can reach a real user via a Filament notification or a Livewire flash.** This is the same principle as rule 72's cycle-guard, generalized: a `static::creating()`/`static::saving()` invariant guard, or a validation-style `throw` inside an Action class's `execute()`, is not "developer-only" just because it lives outside a Form/Rule — Filament and Livewire both surface an uncaught exception's message directly to the acting admin/user. Confirmed bug instances (2026-08-26): `Profile`/`Permission`/`TaskDetail`'s new `static::creating()` duplicate-row guards (`app/Models/{Profile,Permission,TaskDetail}.php`) and `Department::wouldCreateCycle()`'s guard (`app/Models/Department.php`) were all first written with a hardcoded English `RuntimeException` message; `CreateTaskAction`/`UpdateTaskAction`'s Jalali-date guard (`app/Livewire/Dashboard/TaskBoard/Actions/*.php`) threw a raw English `InvalidArgumentException('Invalid Jalali date.')` reachable straight from the task deadline picker. Fix pattern: add (or reuse) a `validation.<key>` entry in that module's `lang/fa/resources/<module>/strings.php` and wrap the throw: `throw new RuntimeException(__('resources/<module>/strings.validation.<key>'));`. Reuse an existing key when one already covers the identical message rather than duplicating text — `Department`'s form-level `App\Rules\NoCyclicDepartmentHierarchy` already had `resources/department/strings.errors.cyclic_hierarchy`; the model hook was fixed to call the same key instead of inventing new text, so the form-rejection and the model-guard-rejection show the identical sentence.

**Gotcha found while fixing the above — a second top-level array key with the same name in a lang file silently overwrites the first, with no error.** Adding `'validation' => ['duplicate_detail' => '...']` as a new block at the bottom of `task/strings.php`, when a `'validation' => [...]` block already existed earlier in the same file (with `user_id`/`attachments.max_size`/etc., genuinely read by `CreateTaskAction`/`UpdateTaskAction`), silently discarded the entire first block — PHP array literals just let the later duplicate key win, no warning, no exception. Every key under the first `'validation'` array (including ones live-called elsewhere) went missing from `__()`'s perspective the moment the page/action rendered. **Rule: before adding a new top-level key to any `lang/fa/resources/*/strings.php` file, search the file for that exact key first — merge into the existing array, never append a second block with the same name.**

**Do not add `Livewire::test()` assertions on the exact persisted content of a `->visible()`-gated dot-notation field** (`assertDatabaseHas`/reading `$model->fresh()->metadata['key']` after `fillForm()->call('create'|'save')`) — the chained multi-step `fillForm()` flow has its own separate staleness behavior (see the static-cache gotcha this rule used to describe, still real, just not the cause of the bug above) and is not reliable for this specific assertion shape. Assert `->assertHasNoFormErrors()` + row existence for the admin-form layer, and cover the actual persisted-value business logic (accessors, scopes, validators) against directly-constructed/factory models instead (see `tests/Feature/Models/ResourceModelTest.php`). Reading existing data via `assertFormSet()` (hydration only) and reading rendered HTML via `->html()` (proved essential for catching the `Get`-resolution bug above) are both reliable.

**74. A validation rule on a root-level field can gate against a value living inside a `Section::relationship('detail')` companion tab — read it via `$get('detail.state')` from the root-level field's `->rule()` closure, and make the gate transition-aware with a `$record` guard so legacy rows that already satisfy the invariant aren't blocked on unrelated edits.** This is the cross-relationship-container form of Rule 37's cross-field closure: the gate is a root-level field (`status`), but the dependency (`detail.state`) lives under a `->relationship('detail')` `Section` (Rule 31), so Filament nests it at state path `detail.state` (same path-nesting as the `fillForm(['detail' => ['checklist' => …]])` test rule above). Return `fn(Get $get, $record) => function(string $attribute, mixed $value, Closure $fail) use($get, $record){…}` and attach to the root field via `->rule(self::doneRequiresState())`. Transition-awareness: the user-panel counterpart only blocks the *transition to* Done (a drag to the done column), not edits on an already-done row — mirror that server-side by short-circuiting when `$record && $record->status === TaskStatus::Done->value` (the record was already done before this edit, so an unrelated field change on a legacy done-without-state row must not fail), and when `$value !== TaskStatus::Done->value` (not transitioning to done) and when `!empty($get('detail.state'))` (state is being set alongside). Only the genuine create-or-edit-transition-to-done-without-state case calls `$fail(__('resources/task/strings.validation.done_requires_state'))`. On Create `$record` is null, so a new task saved as Done without `detail.state` is blocked — matching the user panel. **Rule: a cross-container dependency gate reads the nested state path, and a gate that mirrors a user-side "transition-only" guard carries a `$record` short-circuit so it doesn't retroactively fail pre-existing rows.** Reference: `TaskFormPresenter::status()` + `doneRequiresState()`; mirrors the user-panel drag-to-done guard in `taskboardPattern.md` §14 row 9.

**75. `ProjectResource`/`TaskResource` v2 parity — settings, approval, universal meta, deadline cap — and the centralized-updating-hook rule.** `ProjectResource::form()` carries a `ProjectFormPresenter::settings()` Section (Toggle `requires_approval`, TextInput `sla`, DatePicker `deadline`, Repeater `custom_schema` — same `^[a-z0-9_]+$` key regex as the user panel); `infolist()`/`table()` carry the matching `settingsSummary()` (table column toggleable, default hidden). `TaskResource::form()`'s `tab_bi` carries `TaskFormPresenter::meta()` (`KeyValue`, always shown, never gated on a linked project); `infolist()` carries `approval()` (badge: pending / تأیید date+approver / بدون نیاز), `meta()` (schema-label-or-raw-key fallback, mirrors the card's), and `lastTouchedBy()` (a per-record fallback query — admin's infolist is single-record, so it doesn't reuse the board's batched `LastTouchResolver`, it just queries once for that one record); a record-level `approve` action (first position, icon-button, `visible()` gated on `isPendingApproval() && TaskAccessPolicy::canApprove()`) writes through the same `ApproveTaskAction` the user panel uses. **Rule: approval routing on status-change to `done` lives in exactly one place — `Task::updating()` — never re-implemented in a Filament `mutateFormDataBeforeSave`/Page hook.** Admin and user both go through `$record->update([...])`/`Task::create`, so the model hook fires identically regardless of which panel wrote the row; this is why the admin `EditTask` page needs zero approval-specific code beyond the `approve` action itself. Full mechanism: `app/Livewire/Dashboard/TaskBoard/taskboardPattern.md` §19.

**76. First `openUrlInNewTab()` on a record `Action`, not a Table/Infolist column.** `TaskResource`/`ProjectResource`/`UserResource`'s `recordActions([...])` each carry `Action::make('tasksheet')->url(fn($record) => route('tasksheet', [...]))->openUrlInNewTab()->iconButton()`, opening that record's owner's Tasksheet performance-report page in a new tab; `UserResource` additionally has `Action::make('shareTasksheet')` triggering `App\Services\ProjectTask\TasksheetShareService` to send a share-with-manager notification on the admin's behalf. Every prior `openUrlInNewTab()` call site in this codebase is a `TextColumn`/`TextEntry` (`AdTablePresenter`, `AdInfolistPresenter`, `ProfileInfolistPresenter`, `DmsInfolistPresenter`, `ReleaseRequestInfolistPresenter`, `ContactInfolistPresenter`) — this is the first time it's chained onto a `Filament\Actions\Action` record button instead. Full Tasksheet feature writeup (report structure, scorecard, sharing flow) lives in `app/Livewire/Dashboard/Tasksheet/tasksheetPattern.md` — not duplicated here.

**77. A badge `TextColumn` that joins a variable-length list (multi-department audience/tags/etc.) truncates the DISPLAYED text at 2 items with a `+N مورد دیگر` suffix, but `->tooltip()` always shows the FULL untruncated list — never truncate both, and never truncate only the tooltip.** The table cell has fixed width and the badge can only grow so far before it wraps/overflows the row or pushes other columns; the tooltip has no such constraint, so it's the one place the complete list belongs. Shape: `getStateUsing()` builds the full `$labels` collection, then `$labels->count() > 2 ? $labels->take(2)->join('، ') . ' +' . convertToPersian($labels->count() - 2) . ' مورد دیگر' : $labels->join('، ')`; the sibling `->tooltip()` closure does the SAME lookup but always `$labels->join('، ')` with no truncation branch — it is not "the same string, just longer," it is built independently so a future edit to one doesn't silently also change the other. `count() > 2` (not `>= 2`) means exactly 2 items still shows both in full with no "+N" — the truncation only kicks in from the 3rd item on. Reference: `GalleryTablePresenter::department()` / `ReportTablePresenter::audience()` — both multi-department badge columns, fixed together 2026-09-11 (found via a user report that a many-department gallery/report row's badge became unreadably wide). **Rule: any future badge column joining an unbounded list (tags, recipients, shared-with lists) gets this same two-tier shape — short truncated badge text, full list only on hover — never a single un-truncated join left to overflow, and never a truncated tooltip that hides information the user specifically hovered to see.**
...
**Count Up** (`xplodman/filament-count-up`) is registered globally on the `admin` panel (`AdminPanelProvider->plugins([CountUpPlugin::make()])`): every `Filament\Widgets\StatsOverviewWidget\Stat::make()` animates automatically — `ModuleAnalyticsWidget`'s all-tabs stats included, zero per-widget edits, ever. Numbers inside plain strings (e.g. `"{$males} ┆ {$females}"`) animate token-by-token with `┆`/`%`/ms left as static text; `Htmlable`/`null` values pass through untouched, and it renders the final number server-side first (no-JS-safe, `prefers-reduced-motion`-aware). Package defaults (1000 ms, `,`/`.` separators) are deliberately kept — don't publish its config unless a real need appears. For tables, numeric count columns use `CountUpColumn` (extends `TextColumn`; keeps sortable/badge/color; coerces null → 0, so never use it on a column whose placeholder must survive a null): all 26 `_count`/`combined_read_count` columns across 15 table presenters are converted and every new count column should start as `CountUpColumn`, not `TextColumn`. Gotcha: batch-inserting the `use Xplodman\CountUp\Tables\Columns\CountUpColumn;` import via Git Bash `sed` silently **strips the backslashes** (`use XplodmanCountUpTablesColumnsCountUpColumn;` — unqualified, "Class …\CountUpColumn not found" only at render time, 70+ ViewExceptions suite-wide). Use the Edit tool or `perl -i -pe` for backslash-carrying insertions.

**Evaluated and rejected:** `giacomomasseron/filament-async-column` was installed, demoed live on DMS `users_count`, judged not useful for this app (every derived value is already a SQL-side aggregate, and AsyncColumn would drop sorting), and **removed**. Don't re-adopt unless a genuinely API-bound, non-sortable column appears.

## `PersianDateFieldService` — `->live()` only on `month`, date assembled at save-time not per-pick (production timeout fix, 2026-09-23)

Every year/month/day select this service built (`app/Services/PersianDateFieldService.php`) had `->live()` unconditionally, so picking a year, month, OR day each triggered a full Livewire round-trip — 3 round-trips to fill one date. On the admin Profile edit form (the heaviest form in the panel — repeaters, attachments), those round-trips stacked and occasionally hit an HTTP timeout in production, with nothing in `laravel.log` because the request was dropped mid-flight (proxy/PHP-FPM killed it before Laravel's own exception handler ran). **Fix: `->live()` now lives only on `monthSelect()`** — the only field whose value another field's options closure actually reads (`daySelect()`'s `dayOptions((int)$get("{$prefix}_month"))`). Year and day picks now cost zero round-trips. The real date is assembled once, at save/validation time, via `dehydrateStateUsing` + `mutateStateForValidationUsing` on the Hidden carrier field (both call the same shared closure, now `PersianDateFieldService::assemble()`, made `public`) — replacing the old `afterStateUpdated`-driven per-pick sync that used to write into the hidden field on every select change.

**This is used by 15+ resources (Event, Gallery, Ths, Reservation ×2, Report ×2, Reminder, Task, Profile ×5) — three of them had a real, independently-confirmed correctness bug hiding behind the old per-pick sync, all fixed:**

1. **`TaskFormPresenter::deadlineDate()`'s `ProjectDeadlineCap` rule stopped validating the picked date.** Filament validates a field BEFORE dehydration runs (`vendor/filament/schemas/src/Concerns/HasState.php::getState()` — `$this->validate()` first, `$this->dehydrateState()` after), so a `carrierRule` on the Hidden field was checking a blank/stale pre-edit date instead of the new year/month/day selection once the per-pick sync was removed — an over-cap deadline could silently save, or a valid one could be wrongly rejected on edit. Fixed by mirroring the same assembly closure into `mutateStateForValidationUsing`, so ANY consumer passing a `carrierRule` gets the correct value at validation time too, not just at dehydration.
2. **`ReservationFormPresenter::isRangeState()` read the assembled bare `{prefix}_date` field reactively** (gating `isFullDay`'s disabled-state AND `isRecurring`/`recurCount`/`recurPattern`'s visible/required-state) — since that bare field is now only ever populated at save time, `isRangeState()` read as permanently empty throughout the whole editing session, breaking all four fields' reactive UI logic (functionally, not just "stale" — it never updated at all pre-save). Fixed by rewriting `isRangeState()` to read `{prefix}_year`/`{prefix}_month`/`{prefix}_day` directly and call `PersianDateFieldService::assemble()` itself, rather than trusting the bare assembled field — this still only refreshes on a `month` pick (the only live-triggering field), same inherent Filament limitation as everywhere else, but is now CORRECT on the round-trips that do happen instead of permanently stale.

3. **`ProfileFormPresenter::skills()`'s `last_used_at` silently stopped saving** — found by an independent Fable-5 regression pass, not the original review, since it's a different call path than the other two: the `skills` Repeater's `saveRelationshipsUsing` reads `$component->getState()` directly (raw Livewire state, the field-level path — `vendor/filament/schemas/src/Components/Concerns/HasState.php` — which never runs `dehydrateStateUsing`, unlike the schema-level Repeater save path other date-bearing repeaters on this form use, e.g. `details()`'s `value_date`, which stays correct). `$item['last_used_at']` was reading the bare hidden field's raw value, which the old per-pick `afterStateUpdated` used to keep in sync via `$set()` — now that assembly only happens at dehydration, that raw key is never written during interaction, so every skill saved with `last_used_at = null` regardless of what was picked. Fixed the same way as Reservation's `isRangeState()`: assemble from `$item['last_used_at_year']`/`_month`/`_day` directly in `saveRelationshipsUsing` instead of trusting the bare item key.

**Rule for any future consumer:** if a `carrierRule`, ANY other field's `visible`/`disabled`/`required` closure elsewhere in the same form, or a `saveRelationshipsUsing`/raw `$component->getState()` read on a Repeater embedding this service, needs this component's assembled date value — during live interaction OR at a save path that bypasses schema-level dehydration — read the underlying `{prefix}_year`/`{prefix}_month`/`{prefix}_day` fields directly via `PersianDateFieldService::assemble()` — never the bare `{prefix}` field, which is dehydration/validation-time-only now. Swept all 15 consumers at fix-time (Reservation, Task) plus an independent Fable-5 pass after (Profile skills); no other consumer has this pattern.

**`dehydrateStateUsing`/`mutateStateForValidationUsing` guard against wiping an untouched existing value:** both now share one `$assembleOrKeep` closure that returns the Hidden field's current `$state` unchanged when year/month/day are ALL blank (rather than assembling `null` from three empty values and overwriting whatever was hydrated) — otherwise editing a record without touching this date field at all would blank it out on save. This also guards carrier-direct writes: without the fallback, ANY writer that sets the bare field directly (Filament's test `fillForm(['deadline_date' => ...])`, or any code path writing the carrier without the selects) got silently clobbered to `null` at BOTH validation and dehydration, disabling `carrierRule` checks. That exact regression was caught by `TaskResourceTest::test_create_task_rejects_deadline_past_project_cap` (fillForm sets the bare `deadline_date` only) — that test family is the standing guard for carrier-direct-write semantics.

**Day-clamp on month change (same fix, same day):** picking day 31 then switching month to a 30-(or 29-)day month used to leave the day selector showing an invalid "31" — `Jalalian::fromFormat` does NOT throw for this (empirically verified: `Jalalian::fromFormat('Y/m/d', '1403/07/31')` silently rolls over to `1403-08-01`, no exception, in both the old per-pick-assemble code and the new save-time-assemble code — a claim that this became a save-time crash was checked and found false), it silently produces the WRONG date one month later than intended. Pre-existing bug, not something today's `->live()` change introduced or worsened — fixed anyway since it's free: `monthSelect()`'s `afterStateUpdated` (already firing on every month pick) now clamps `{prefix}_day` down to `daysInMonth($newMonth)` if it exceeds it.

**Why none of this was caught by the existing test suite (a real gap, not fixed here — flagged for the next test pass):** `TaskResourceTest`'s `ProjectDeadlineCap` tests either set the bare `deadline_date` key directly via `fillForm` (bypassing the year/month/day selects entirely, so the bug path never executes) or omit `project_id` on the year/month/day-based case (so the cap rule short-circuits before it would even notice a wrong date). Reservation's `isRangeState()`-driven reactive UI state (the `isFullDay`/`isRecurring` disable/visible logic) has no test coverage at all. Both are the same root cause: Filament resource tests conventionally use `fillForm([...])` to set the WHOLE form state in one shot, which never exercises the actual multi-step "pick month, THEN pick day" interaction sequence a real admin performs — the exact sequence that both bugs (and the original 3-round-trip performance issue) only manifest through. A future test for either bug needs to `set()` the `_year`/`_month`/`_day` sub-fields individually (not the bare `fillForm(['deadline_date' => ...])` shortcut) to actually exercise the live-interaction path.

## Post-fix sweep — the `->live()`-with-no-consumer bug recurs elsewhere (2026-09-23)

After the `PersianDateFieldService` fix above, a 4-way parallel audit (form round-trips, page lifecycle hooks, table/RelationManager N+1, app-wide DB-hostile patterns) plus a manual review found the same bug class in **6 more places**, all fixed the same way — remove `->live()`, confirmed zero downstream `$get()` consumer in each case before touching:

- `ThsResource\Schemas\TicketFormPresenter::requestArea()` — terminal field of a 3-level cascade (`targetDepartment` → `requestType` → `requestArea`), nothing reads its value.
- `EventResource\Schemas\EventFormPresenter::countdownMood()` and `countdownConfetti()` — both live, nothing reads `mood`/`confetti`; only the sibling `countdownEnabled()` toggle is a real dependency.
- `LinkResource\Schemas\LinkFormPresenter::linkType()` — nothing reads `$get('link')`.
- `DepartmentResource\Schemas\DepartmentFormPresenter::level()` — its own `options()` reads `subordinate_to`, not `level`; nothing reads `level` itself.
- `OnboardingResource\Schemas\OnboardingFormPresenter::guides()`'s `FileUpload::make('url')` — no consumer; Livewire's own upload mechanism doesn't need field-level `->live()`.
- `ReservationResource\Schemas\ReservationFormPresenter::cancelReason()` — its own `visible()` correctly depends on `status`, but nothing depends on `cancel_reason` itself.

**Separately, an unrelated guaranteed-fatal bug was found and fixed in the same sweep:** both `ResourceResource` and `UserResource`'s `ReservationsRelationManager::form()` called three `ReservationFormPresenter` methods that no longer exist — `fullDayDate()`, `startTime()`, `endTime()` — leftover from before the reservation-wide migration to `PersianDateFieldService` (see `reservationPattern.md`'s "Admin form date handling" section). Opening the "create" table action on either RelationManager was a guaranteed PHP fatal error. First remapped to the real field set as a stopgap, then resolved the same day the ERP-native way: **both RMs now declare `protected static ?string $relatedResource = ReservationResource::class;` and their `form()`/`infolist()` overrides are deleted.** Filament's `RelationManager::getDefaultActionUrl()` turns their Edit table action into a plain URL link to the real `EditReservation` page (where `FilamentDateHandler` runs; no CreateAction exists on either RM table), and the ViewAction (no `view` page exists) stays a modal but renders `ReservationResource`'s own form/infolist through vendor delegation (`InteractsWithRelationshipTable::form()`/`infolist()` call `ReservationResource::form()/infolist()` when `$relatedResource` is set) — so no RM-level schema duplication remains to drift again. Both RM tables also carry `->toolbarActions([])` (user decision 2026-09-24): the delegation would otherwise inherit ReservationResource's bulk delete/exporter, which belongs on the main reservations table only — the RM `table()` runs after `$relatedResource::configureTable()`, so the empty-array override wins; the tabs stay plain list/edit tabs. The RMs' redundant `getModelLabel()`/`getPluralModelLabel()` overrides (identical to what delegation supplies) are deleted; each keeps its distinct `getTitle()`. This is the ERP norm for related-record surfaces: a modal must never present fields it cannot persist — the duplicated RM form couldn't hydrate the `{field}_date`/`{field}_time` parts on edit (`EditAction` fills from `$record->attributesToArray()`, and those keys aren't record attributes) and silently dropped them on save (not fillable), so an admin could "edit" dates that never saved while the required hidden carrier blocked other saves. Regression tests: `ResourceResourceTest`/`UserResourceTest` `test_reservations_relation_manager_edit_navigates_to_the_full_reservation_edit_page` asserting the edit action's URL via `assertTableActionHasUrl` — the old `mountTableAction('create')` smoke test was silently vacuous (`mountAction()` swallows a non-resolvable action with a null return, so it passed even when the action never existed and even while the form fataled). **Gotcha for future date-field refactors:** grep for every static-method call site across `Pages/`, `RelationManagers/`, and `Schemas/` in every resource before renaming/removing a `*FormPresenter` method — a `RelationManager` reusing the same presenter is easy to miss since it lives in a different resource's directory tree, and unlike the main resource form, nothing exercises it until an admin actually opens that tab.

**Findings surfaced above — resolved 2026-09-23, one per-issue, all reviewed by an independent Fable-5 pass:**

1. **`ReservationFormPresenter::parentId()`** — was `Reservation::roots()->with(['resource','user'])->get()`, unbounded, re-running on every live field. Fixed with Filament's native async search instead of a flat row cap: `->getSearchResultsUsing()` (matches by id prefix, resource name, or user name — NOT the decorated `resource_dropdown_label` string, which has emoji/Jalali-date noise no SQL `LIKE` could match — `latest('start_time')->limit(50)`) + `->getOptionLabelUsing()` (resolves a single saved id back to its label so an Edit form pre-fills correctly without first typing a search term). Zero eager load, zero cache, admins can still reach any reservation by typing instead of scrolling a capped list.
2. **`HasTicketOptions::getCustomAdminAreaLabelHtml()`** — was `Blade::render()` (a real Blade compile) per option, ~15 times per render. `<x-icon>` in this app resolves to `blade-ui-kit/blade-icons`' generic `Icon` component (`BladeUI\Icons\Components\Icon`, confirmed via vendor source — **not** Filament's own `generate_icon_html()`, a claim that was checked and found wrong before implementing), which itself just calls the global `svg($name, $class, $attributes)` helper. Replaced the whole `Blade::render()` call with a direct `svg($icon, $class)->toHtml()` + plain string concatenation — identical output, zero Blade compiles, `Blade` facade import removed (last use in the file).
3. **`TicketFormPresenter::assignedTo()`** — uncached department-scoped query on a form with 5 live fields. Fixed by adding `User::getCachedDepartmentOptions(string $dept)` mirroring `getCachedAllOptions()`'s existing pattern, keyed `"user_dept_options:{$dept}"` (parameterized per `modelPattern.md` rule 5). **Invalidation completed same day:** the cache's write path lives on `Profile` (`department_id`), which `User`'s own `booted()` gate never sees — `Profile::booted()` now bumps `ModelCacheVersion::bump(User::class)` on `wasChanged('department_id')` (creation included — `getChanges()` contains the attribute on insert) and on `deleting` (a deleted profile removes the user from the dept list), guarded by `UserModelTest::test_moving_a_profile_between_departments_invalidates_cached_department_options`.
4. **`ProjectTablePresenter::progress()`** — was `getStateUsing` calling `ReportingService::summary()` per row (up to 4 queries × visible rows on a cache-cold load). Fixed by adding a third `withCount(['tasks as done_tasks_count' => ...->where('status','done')])` alongside `ProjectResource`'s existing `withCount('tasks')`/`overdue_tasks_count`, then deriving percent inline (`done_tasks_count / tasks_count * 100`, same `round(...,1)` + zero-task guard as the original `summary()` formula) — zero queries per row, whether the column is shown or hidden, and the `ReportingService` import/call is gone from this file entirely (the service's own cached `summary()` method is untouched — still used elsewhere, e.g. `TasksRelationManager`).
5. **`LinkFormPresenter::internalUrl()`** — was `->live()` (per-keystroke round trip); its only consumer, `companyIps()`'s `visible()`, doesn't need pre-blur reactivity. Changed to `->live(onBlur: true)` — one round trip per edit instead of one per few characters typed.
6. **`UserFormPresenter`'s booking-toggle repeater** — no minimal fix exists (a client-side rewrite would need to reimplement Filament's entangled Toggle state from the DOM); left as-is, each toggle still round-trips, which is a genuine dependency (the "all" toggle needs bidirectional sync), not the zero-payoff bug class.
7. **`ProfileFormPresenter`'s `details.key` select** — legitimately `->live()` (correct, not touched), but re-hydrates the panel's heaviest form (5 repeaters) on every pick. Fixed the actual cost instead of the liveness: added `->collapsed()` to the four SIBLING repeaters (`about_me()`, `attachments()`, `favorite_colors()`, `skills()`) — `details()` itself deliberately left uncollapsed (it's the one being edited). A collapsed repeater item skips rendering its child form entirely and renders only its header row, so a `details.key` round trip now re-renders four header rows instead of four full repeater bodies. No change to validation/hydration/save — Filament still computes and hydrates collapsed items' state, it just doesn't render them open.
8. **`SkillTablePresenter::categoryFilter()`** — was an uncached full-table `DISTINCT` pluck on every filter-panel render. Added `Skill::cachedCategoryFilter()` mirroring the file's own existing `cachedActiveCatalog()` pattern (direct `Cache::remember(ModelCacheVersion::key(...))`, not the `HasModelCache` trait, to match this specific file's established style) — `now()->addDay()` TTL, Tier V (auto-invalidates on any `Skill` save/delete).
9. **`Photo::allDepartmentModels()` / `Report::audienceDepartments()`** — both re-computed 3-4× per table row by separate column closures (`color()`, `icon()`, `getStateUsing()`, `tooltip()`). Added `->shouldCache()` to both `Attribute::make(...)` accessors, matching sibling accessors already doing this in the same files — per-instance memoization, safe since nothing mutates `departments`/`all_departments` mid-request in either model's write paths.
10. **`NudgeService::reconcile()` / `EdgeService::reconcile()`** — both had a per-recipient `create()` inside a `foreach` over potentially every active employee (a broadcast trigger like `Post`/`Feed`/`Ad` fires for `User::active()->get()`). Fixed by accumulating genuinely-new rows into an array and writing them with one bulk `insert()` after the loop — mirroring the pattern `BadgeSyncService::syncBatch()` already uses for this exact table (`app/Services/Menu/statePattern.md` documents both). **Non-obvious wiring requirement, easy to miss:** raw `insert()` bypasses Eloquent's `saved` event, which is what normally bumps `DatabaseNotification`'s Tier-V cache backing `UnreadNotifications`'s per-user `unread_menu_groups:{userId}` badge count (`modelPattern.md` rule 6: "bulk/raw writes ⇒ one manual `bump()`") — without an explicit `ModelCacheVersion::bump(DatabaseNotification::class)` after the insert, every recipient's unread badge would sit stale for the full 15-minute TTL after any broadcast nudge. `EdgeService`'s bulk insert needs no such bump — verified no cache anywhere is keyed on `Edge`. Full detail in `statePattern.md`.

## Admin panel loading indicator — do not hand-roll one

The admin panel's global page loader is `resources/views/components/ui/loaders/page-transition.blade.php`, wired into `app/Providers/FilamentServiceProvider.php`'s `PanelsRenderHook::BODY_END` hook — a `.global-app-loader` fixed overlay wrapping `<x-ui.loaders.spin-badge>`. Full rule set (including the one deliberate exception for non-Livewire Alpine/`fetch()`-driven action buttons) lives in `resources/views/components/ui/loaders/loadingPattern.md`. **Grep for `view('components.ui.loaders...` (a PHP-side `view()` call, not a Blade `x-ui.loaders.*` tag) before deleting or renaming any shared loader component** — a PHP-side render-hook consumer like this one won't show up in a Blade-only grep, and missing it caused a full admin-panel outage on 2026-08-24 when `spinner.blade.php` was deleted without checking `FilamentServiceProvider.php` first.
