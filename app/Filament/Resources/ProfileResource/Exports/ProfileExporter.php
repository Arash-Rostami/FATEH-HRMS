    ...
    public static function getColumns(): array
    {
        return [
            ...
            ExportColumn::make('details')
                ->label(__('resources/profile/strings.export.details'))
                ->state(fn(Profile $record): string => $record->details->isNotEmpty()
                    ? $record->details->map(fn($d): string => $d->label . ': ' . $d->display_value)->implode(' | ')
                    : '-'
                ),
