...
class ReservationExporter extends Exporter
{
    ...
    {
        return $query->with(['user', 'resource']);
    }
    ...
    public static function getColumns(): array
    {
        return [
            ...
            ExportColumn::make('start_time')->label(__('resources/reservation/strings.export.start_time'))->formatStateUsing(fn($state) => $state ? toJalaliSmart($state) : '-'),
            ExportColumn::make('end_time')->label(__('resources/reservation/strings.export.end_time'))->formatStateUsing(fn($state) => $state ? toJalaliSmart($state) : '-'),
