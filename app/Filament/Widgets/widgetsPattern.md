...
**Helpers.** The shared `DepartmentAxis` trait (below) exposes only `topDepartments()`. `positionAxis`/`positionMap`/`bucketSeries` are private to `HrAnalyticsService` (relocated there from the widgets, not widget-local). Widget-local helpers stay private and descriptively named (`getScopeCondition`, `activeModule`).
...
| `ModuleAnalyticsWidget` | -1 | custom Widget | 28 `{domain}Data()` stat builders, tabbed, view `livewire.admin.widgets.filament-analytics` |
| `ModulesGuideWidget` | 0 | custom Widget | `config('modules')`-driven module guide, view `filament.widgets.guide` |
...
Lower `$sort` = higher on the page. Utility widgets occupy the negatives; `ModuleAnalyticsWidget` was moved from `6` to `-1` (2026-08-26) so the dashboard's stats sit right below the preferences accordion, near the top, instead of after the HR/operations chart blocks — `ModulesGuideWidget` shifted from `-1` to `0` to make room. The HR block sits at 1–4; the operations/org-analytics block at 5, 7–8 (gap at 6 is harmless, sort need not be contiguous).
...
- **Single-query bucketing over derived columns.** Age/tenure are computed once in a derived subquery (`(SELECT … TIMESTAMPDIFF(YEAR, birthdate, NOW()) AS age …)`) then bucketed with `SUM(CASE WHEN age < 25 THEN 1 ELSE 0 END)` in one `groupBy` pass — not per-row `TIMESTAMPDIFF` repeated 5×. See `getHrCData`, `getHrFData`, `getHrGData`, `getHrKData`; `getModuleIData` is the same single-query bucketing but inline (`selectRaw` with `TIMESTAMPDIFF` in each `CASE`, no derived subquery).
- **Derived aggregates instead of correlated subqueries / cartesian fan-up.** `OperationalChart::getModuleAData` and `StructuralChart::getModuleEData`/`getModuleHData` use `leftJoinSub` for per-department task/ticket/energy/profile/report counts — computed once, joined once. The base `departments` table drives the row set; where a top-N row set is needed, `LIMIT 10` gets a deterministic `orderBy('departments.id')`/`orderBy('departments.code')` tie-breaker.
- **Single-pass 2D accumulation.** `HrAnalyticsService::getHrNData` builds the dataset array up front (one slot per `PresenceStatus::cases()`), then accumulates per row in one pass; `HrAnalyticsService::bucketSeries()` does the same for known-enum datasets plus one trailing rest dataset. See rules 59 (COW alias trap) and 60 (nullsafe on grouped-row lookup) in `filamentPattern.md`.
- **Null/invalid bucketing.** Enum/bucket columns never silently drop rows — null/invalid values land in a trailing `__other__` / `سایر` / `نامشخص` dataset appended only when such rows actually exist. `HrAnalyticsService::positionAxis($rows)` appends `__other__` only when a null/invalid position is present; `positionMap()` is the matching validity map (both relocated from `HrQualificationChart` to the Service).
...
A custom `Widget` (not `StatsOverviewWidget`) rendering `livewire.admin.widgets.filament-analytics`. Holds `public string $activeTab` (Livewire tab state) and 28 `#[Computed(seconds: 300, cache: true)]` `{domain}Data()` builders returning arrays of `Filament\Widgets\StatsOverviewWidget\Stat`. **This `#[Computed(cache: true)]` genuinely caches — but only because every method is consumed as a bare no-argument property** (`$this->usersData`, `$this->adsData`, … in `getActiveStatsCount` / `statsSchema`), which routes through `BaseComputed::handleMagicGet`. That is the *one* shape where the attribute works; a method called directly (`$this->getModuleAData($dept)`) bypasses it — which is why the chart widgets (`StructuralChart`/`OperationalChart`/`Hr*Chart`) use `ModelCacheVersion::rememberGlobal` instead, not `#[Computed(cache: true)]` (see `filamentPattern.md` rule 25). `departmentsData()` reads `Department::getCachedModels()->count()` for the total + one `withCount('users')` (excluding `Guest` type only — VIP counts) + collection filter. Lazy-loaded.

**MANDATORY: every new admin Resource gets a tab here — extend, don't forget.** When a new `*Resource` is added under `app/Filament/Resources/`, add its stats to `ModuleAnalyticsWidget` in the same pass (treated like the `FilamentAdminGuide`/legend fan-out — see `filamentPattern.md` §3.4): (1) one `#[Computed(seconds: 300, cache: true)]` `{domain}Data(): array` method — a single `DB::table('table')->selectRaw("COUNT(*) as total, SUM(CASE WHEN ... THEN 1 ELSE 0 END) as ...")->first()` aggregate, returned as an array of 3–4 `Stat::make(label, value)->icon(...)->color(...)`, reusing the resource's own `strings.plural_label`/`fields.*` translation keys where a matching label already exists, adding new keys under `resources/dashboard/strings.php`'s `analytics.{domain}.*` only for stat labels that have no resource-string equivalent; (2) register it in `getAllTabs()` (icon + label); (3) register it in `getActiveStatsCount()`; (4) register it in `statsSchema()`. Alphabetical method placement within the file. **Consume the method ONLY as a bare no-arg magic-property** (`$this->xData`, never `$this->xData()`) in all three registration points — a method-call bypasses the `#[Computed(cache:true)]` engagement entirely (see the cache-engagement note above). Skip only when the resource has no meaningful aggregate of its own — a pure pivot/junction resource already folded into its parent's tab doesn't need a duplicate (`SkillRequestResource`, model `SkillUser`, is covered by the existing `skillsData()`'s `skill_user` aggregate, not a separate tab). Add a `tests/Feature/Filament/Widgets/ModuleAnalyticsWidgetTest.php` `statMethodsProvider()` entry for the new method (smoke-tests it returns a non-empty array of `Stat` instances).
...
One file per widget (`<Widget>Test.php`), mirroring the one-file-per-target rule in `tests/Feature/coreTestPattern.md`. The widget logic is exercised by **direct reflection, not Livewire mounting**: `(new ReflectionClass($cls))->newInstanceWithoutConstructor()` + `ReflectionMethod::setAccessible(true)` to call each `get{Key}Data()` / `{domain}Data()` / private helper (`getModuleIData`; `topDepartments`/`bucketSeries`/`positionAxis`/`positionMap` relocated to `HrAnalyticsService` and reflected there in `HrAnalyticsServiceTest`, not here) directly. The `#[Computed]` cache does not intercept a raw `invoke()`, so a `ModuleAnalyticsWidget` `{domain}Data()` body runs fresh each call. **For the `rememberGlobal`-wrapped chart widgets (`StructuralChart`/`OperationalChart`) the raw `invoke()` *does* run through `Cache::remember`** — so the `Cache::flush()` in setUp/tearDown already isolates each test; to assert the cache itself, call twice on a non-flushed store and assert the second call returns the same payload without re-querying (see `modelPattern.md` "Verification shape"). `getData()` dispatch is tested by setting `$widget->filters` (public `HasFiltersSchema::$filters`) and calling the protected `getData()` via reflection, then asserting it equals the direct data-method call on the same instance. Skeleton is the verbatim `useMysql()` + `beginTransaction`/`rollBack` from `tests/Feature/Filament/WidgetRenderCheckTest.php`, plus `Cache::flush()` in setUp AND tearDown; `ModuleAnalyticsWidgetTest` additionally calls `Once::flush()` alongside `Cache::flush()` (departmentsData reads `Department::getCachedModels()`, which memoizes via `once()` around `Cache::remember` — so `Once::flush()` is still required to reset that memo between tests, alongside `Cache::flush()`).
...
- **Top-10 axis robustness:** `HrUnitHealthChartTest` scales the seeded dept's profile count past the dev-DB top-10 threshold (`topTenThreshold()+3`) and asserts at the seeded dept's own index, so pre-existing dev-DB rows don't make counts non-deterministic. The parallel `maxDeptProfileCount()+3` helper lives in `HrAnalyticsServiceTest` (which covers the hr_n–hr_q axis the same way). `HrEngagementChartTest` tests only dispatch/type/options — no axis scaling.
