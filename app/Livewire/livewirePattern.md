...
- **Chat message smart timestamp** (cross-module — Contact `ContactPresenter::messages`, Channel `ChannelPresenter::messages`): the per-message `time` field uses `$createdAt->isToday() ? $createdAt->format('H:i') : toJalali($createdAt->toDateTimeString(), 'Y/m/d H:i')` — today shows time-only, older days show Jalali date+time. The day-group separator label uses `toJalali($date, 'j F Y')` (Jalali day month year, NOT `translatedFormat('j F Y')` which renders Gregorian with a translated month name). Keep `امروز`/`دیروز` for today/yesterday. The `datetime` (ISO) field is unchanged for the `<time datetime>` attribute.
- **User-panel display standard — uniform `j F Y`**: user-panel date displays default to `toJalali($value, 'j F Y')` (Persian day-month-year), with deliberate carve-outs: **(a) precise timestamps** keep full time — Contact/Channel chat message balloons + read-receipt tooltips (`Y/m/d H:i`), Profile document upload (`Y/m/d H:i`), and the error page (`Y/m/d H:i:s`, seconds kept for max precision); **(b) shorter functional forms** that are load-bearing — Gallery month filter keys (`F Y`, the `x-show`/dropdown `key` the timeline filter matches) and Reservation calendar day labels (`j F`, year implicit within a single month view); **(c) relative/presence** via `toJalaliRelative()`/`since()` for last-seen/online (a separate bucket, not an absolute date). Do not introduce a new format string outside these carve-outs — use `j F Y`. The admin panel has its OWN standard (`toJalaliSmart` via the `HasJalaliAdminLabels` trait; see `filamentPattern.md`); the two panels intentionally differ.
- **Chat read-receipts** (cross-module — Contact 1:1, Channel group): for `is_mine && is_last` messages, the presenter emits `readers` (array of `['avatar','name']`, up to 3), `read_count`, `is_read` (`read_count > 0`), `is_read_by_all` (`read_count === total_members`, where `total_members` is an internal non-sender counter that is `0` when no other members exist, so the boolean is false on empty membership — not an emitted field). The readers map is built by ONE memoized query in the component (`Main::readers()` for Channel; contact uses a per-message `read_at`), passed into the presenter's `messages()`/`messageGroup()` signature — never queried inside the presenter loop. The blade renders a `done_all` (primary when `is_read_by_all`, muted when `is_read`), `done` (muted when not read), plus an avatar stack (up to 3 `<x-ui.avatar>` + `+N` chip). Zero non-sender members → no check/avatars (vacuous-truth guard).
- **Cross-panel consistency**: if a model exposes its own formatted-date accessor (e.g. `Task::createdFormatted`/`deadlineFormatted` via `$appends`), that accessor remains the **user-panel** source of truth — Blade reads it directly (`$task['created_formatted']`, `j F Y`). The **admin** Filament side no longer calls the accessor: it uses the `HasJalaliAdminLabels` trait label (`createdLabel()`/`adminDateLabel()`, `toJalaliSmart`) so the admin is uniform `Y/m/d H:i` while the user panel keeps `j F Y`. The two panels intentionally hold different standards (see the user-panel display standard above + `filamentPattern.md`'s `HasJalaliAdminLabels` rule) — the old `TaskResource` admin (`Y/m/d`) vs Kanban card (`j F Y`) drift was resolved by giving each panel its own format rather than forcing one accessor onto both.
...
The Channels module (`app/Livewire/Dashboard/Channel`) **uses `@island(name:'sidebar')` + `@island(name:'messages')`** per `.claude/plans/channels-master-plan.md`. A previous agent removed islands entirely (treating the symptom); they were restored once the real root cause was traced (first bullet). The traps below are **general guidance for any `@island` use** — reviewers MUST reason about island behavior statically; tinker PASS does not acquit.

- **Island auto-scoping keys off `action.origin`, which only `wire:click`/`wire:model` directives set — `x-on:click` (Alpine) → `this.$wire.method()` does NOT.** The earlier diagnosis "islands don't auto-render on in-island actions" was a misread of this. Livewire's `interceptAction` (`dist/livewire.esm.js:13778`) auto-scopes an action to its enclosing island ONLY if `action.origin` is set; `wire:click`/`wire:model` call `setNextActionOrigin({el, directive})` (the directive's element is inside the island), but an Alpine `x-on:click` that calls `this.$wire.method()` reaches `fireAction` with `outstandingActionOrigin = null` → `interceptAction` returns early → no island metadata → the server does a FULL render → every `@island` whose `always:` is false emits a `mode:'skip'` empty fragment (`HandlesIslands::renderSkippedIsland`) → the island DOM is left untouched. This is why `loadOlder`/`setFilter`/`search`/`removeAttachment` (all `wire:click`/`wire:model`) worked while `send`/`saveEdit`/`deleteMessage`/`undoDelete`/`selectChannel`/navigations (all `x-on:click → $wire.method()`) did not — the DB wrote and `#[Computed]` was unset, but the messages island showed nothing ("I send and don't see it", "select needs a full reload"). **Rule: every Alpine-triggered action that must update an island is explicitly scoped with `$wire.$island('name').method()` — scope the action ITSELF so the method runs and `renderIsland('name')` fires in one request (server `call` hook sees `metadata.island` → `skipRender()` + `renderIsland()`), then chain a refresh of the other island only if its UI also changed.** The Channels pattern: `selectChannel` is `$wire.$island('messages').selectChannel(id).then(() => $wire.$island('sidebar').refreshUnread())` (loads messages + sidebar highlight); `send`/`saveEdit`/`deleteMessage` are `$wire.$island('messages').<method>()` (messages re-render only); navigations (`toggleBrowse`/`openCreate`/`closeCreate`/`backToList`/`leaveChannel`/`createChannel`/`joinChannel`) are `$wire.$island('messages').<method>().then(() => $wire.$island('sidebar').refreshUnread())`. `undoDelete` is scoped in the blade (`x-on:click="$wire.$island('messages').undoDelete()"`) since it has no Alpine wrapper. Do NOT use `$wire.$island('name').$refresh()` — `$refresh()` is a magic action, not a real method; scope a real method (the action itself, or a `refreshUnread`-style custom method) instead. Note `wire:click`/`wire:model` triggers need NO explicit scoping — they auto-scope; only the Alpine `x-on:click → $wire.method()` triggers need `$island()`.
- **Island `$refresh()` runs in REDUCED scope.** On `$island('messages').$refresh()` the island re-renders with: ✅ public properties as bare vars, ✅ `$this` (the component), ✅ `$__livewire`. ❌ bare `#[Computed]` variables are NOT injected — `$activeChannel`, `$messages`, `$groupedMessages`, `$joinableChannels`, `$presenter` are undefined in the island template on refresh. **Rule: inside any `@island` template, access computeds via `$this->` — `$this->activeChannel`, `$this->messages`, `$this->presenter` — never the bare name.** The sidebar island already used `$this->channels`/`$this->presenter` (the proven pattern); the header/info/browse/messages islands had to be converted from `$activeChannel` → `$this->activeChannel` after a real browser island-refresh threw "Undefined variable $activeChannel".
- **`@php(...)` parenthesized form breaks inside islands.** `@php($a = 1; $b = 2)` compiles differently in Livewire's `ExtendedCompilerEngine` island compile path vs a full render, producing "syntax error, unexpected token ;". The block form `@php ... @endphp` compiles identically in both paths. **Rule: inside `@island` templates, always use the block form `@php ... @endphp`, never the parenthesized one-liner `@php(...)`.** The parenthesized form is fine outside islands (line 185), but ban it inside any `@island`.
- **`@entangle` and nested anonymous Blade components (`<x-...>`) do NOT work inside islands.** Nested anonymous components don't inherit `$__livewire`, so `@entangle($model).live` (used by `<x-ui.forms.search>`) fatals inside an island. **Rule: inside `@island` templates, use plain `wire:model` / `wire:model.live` + Alpine `x-model` for inputs, never `<x-ui.forms.search>` or any component that relies on `@entangle`.** The sidebar search is a plain `wire:model.live.debounce.200ms` input for this reason.
- **`Livewire::test()->html()` and tinker full-render MASK every island-only bug.** A full render shares scope globally (computeds are injected as bare vars, `@php(...)` compiles the full-render way, the whole component renders as one tree) — so `Livewire::test(...)->call('send')->html()` shows the new message, the edited body, the restored row, all correct. The island `$refresh()` path is never exercised. **Rule: a tinker/test PASS is not evidence the island works.** Reviewers and the PostToolUse gate MUST reason about island scope and memoization statically — `grep` the island templates for bare computed names, `grep` for `@php(`, `grep` for `@entangle`/`<x-ui.forms.search` inside `@island`, and `grep` mutations for a missing `$island(...).$refresh()` or a `messages-updated`-style dispatch — because the runtime test that would catch these is a real browser island-refresh, which the gate does not perform. This is why 59 reviewers green-lit a module that errored on first browser use.
- **`x-ref` inside `@island` throws on the first island re-render and aborts Alpine's whole init pass — "works after refresh, dies after first change."** Registering `x-ref` walks up `findClosest(el, i => i._x_refs …)` to the nearest Alpine root to write `_x_refs[name]`. During an island morph, Alpine's `cloneNode`/`initTree` clones the new subtree while its ancestor chain is mid-detach, the walk returns `undefined`, and it throws `Cannot read properties of undefined (reading '_x_refs')` (Alpine `module.esm.js:3436`, `$refs` magic → `getArrayOfRefObject`). The throw aborts `initTree` partway, so every directive AFTER the ref element (toolbar buttons, `x-on:click` handlers, `x-if` clones further down) never initializes — the island looks frozen even though the morph itself completed. First cold load survives (ancestors attached); the first `send`/`saveEdit`/`deleteMessage` that re-renders the island dies. **Rule: never use `x-ref` inside an `@island` template.** Reach the element by `document.getElementById(...)` (the Channels module's `msg-viewport`/`msg-ta` access pattern in `channel.js`) or, for reactive sibling state, a scoped `<div x-data="{ len: 0 }" x-effect="len = ($wire.composer.body || '').length">` wrapper feeding `x-on:input="len = $el.value.length"` — no ref walk, morph-safe. `grep -rn 'x-ref=' resources/views/livewire/dashboard/channel/` MUST return empty for any island-using module.
- **`<template x-if>` inside `@island` renders dead buttons when the condition is true at morph time.** Alpine's morph plugin clones `<template x-if>` content into the DOM during the island morph but does NOT reliably re-run `x-on:click` handler init on the cloned content — buttons render but clicks do nothing (copy/reply/edit/delete all "don't work"). The trap is specific to x-if conditions that are **TRUE at morph time** (so the content is cloned during the morph, not at a later reactive toggle): the Channels message bubble used `<template x-if="!(isEditing && editingMsgId === {{ $msg['id'] }})">` (true while not editing → toolbox cloned at morph → dead). An x-if that's **FALSE at morph time** (e.g. the composer's `<template x-if="replyingTo">`, false until `startReply` sets `replyingTo`) is fine — its content is cloned later by Alpine's reactive system, which inits correctly. **Rule: inside `@island`, prefer `x-show` over `<template x-if>` for any toggle whose visible branch contains interactive elements (buttons/inputs with `x-on:click`/`x-model`).** `x-show` keeps both branches in the DOM (one `style="display: none;"`), Alpine inits them once, and handlers survive every morph. With `x-show`, replace `x-init` (fires once at mount) with `x-effect` for behavior that must re-run each time the toggle becomes active — e.g. refocus the edit textarea: `x-effect="if (isEditing && editingMsgId === {{ $msg['id'] }}) $nextTick(() => { $el.focus(); $el.selectionStart = $el.selectionEnd = $el.value.length })"`.
...
- **`WithFileUploads` inside `@island` skips the island on upload-finish, so the staged-attachment preview never paints — bridge it with a `->self()` dispatch the island's own Alpine `$wire.on` catches, then re-render via a real island-scoped method (NOT `$refresh()`).** `startUpload`/`finishUpload` are Livewire-internal calls that do not carry island origin metadata, so the upload-finish response is a full render where the island's DOM diff is skipped. The composer property (`composer.attachments`) IS set server-side and the client model IS updated, but the staged-preview `@if(count($composer->attachments))` lives inside the island template — its HTML never morphs in, so the user picks a file, sees nothing, and only sees it appear after `send` (the next full island render). The fix is a two-call dance: (1) a Livewire `updated(string $name)` hook on the root component fires when `composer.attachments` changes and dispatches `attachments-updated` `->self()` — `->self()` events are delivered to the client at component level even when the island DOM was skipped, so Alpine `$wire.on('attachments-updated', ...)` always catches it; (2) that listener calls `$wire.$island('messages').syncAttachments()`, a real non-empty island-scoped method (e.g. `unset($this->groupedMessages)` — non-empty, non-looping, no extra query, matches the `unset($this->X)` computed-invalidation idiom; do NOT touch `composer.attachments` inside it or `updated()` re-fires into an infinite loop), which issues a fresh island-scoped request, the island re-renders with populated `composer.attachments`, and the preview paints. **Never use `$wire.$island('name').$refresh()` here — it's the forbidden magic action; scope a real method instead.** Same trap hits any `wire:model` on a file input living inside an `@island`.
