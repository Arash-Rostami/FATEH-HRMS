...
- **Data** — `event_shares(event_id, user_id recipient, shared_by, timestamps)` with `unique(event_id,user_id)`; `EventShare` model; `Event::shares()` HasMany + `Event::isSharedWith($userId)`. Shares are created **only by the owner** (guarded in `ShareEventAction` and again in `Calendar::openShareModal`/`sharingEvent`), so a recipient has a row only if the owner chose them — the visibility query `where user_id=me OR private=false OR whereHas shares user_id=me` cannot leak a private event to anyone else, and public events were already visible (sharing a public event is redundant but harmless).
