...
- **No invite system** — Private channels are admin-created; only the owner sees them. Open channels are joinable by anyone.
...
| `openCreate()` / `closeCreate()` | createMode flag + reset create form (`type='open'`) | Open-only in user panel (no invite system) |
| `createChannel()` | set `create->type='open'`; call `CreateChannelAction` (which now builds the slug internally via `resolveSlug()` before `validate()`), `selectChannel(new id)`, toast | catch `ValidationException` (toast first error) + `\Exception` (generic toast); slug from Persian name works via `\p{L}\p{N}` regex — slug generation moved out of Main into the action so the form's `slug.unique` rule validates the populated slug in one pass |
...
| `messages(array $messages, int $authId, int $editTimeLimit): array` | per-message: `{id,body,body_html,created_at,time,datetime,is_mine,is_first,is_last,is_edited,is_deleted,sender_name,can_edit,can_delete,attachments,gap_class,bubble_radius,reply_to,animation_delay}`; `can_edit = isMine && no deleted_at && diffInSeconds(now) <= editTimeLimit` |
| private `replyPreview, bubbleRadius, attachments, linkify` | helpers |
...
| `messages.blade.php` | `$hasOlder=$this->hasOlder` | date-grouped bubbles; each row `data-rf="channel-message-{id}"` (focus target for `record-focus`); empty-state uses `$this->activeChannel->name`; shared `<x-ui.buttons.load-more action="loadOlder">` gated by `x-show="$wire.hasOlder" x-transition` (fades on last round); scroll-to-top auto-fires `loadOlder` (channel.js) |
...
| `create.blade.php` | | Open-only create form: `wire:model="create.name"` + `create.description`; submit `createChannel`; cancel `closeCreate`. No type field (hardcoded `open`). |
| `empty.blade.php` | | no-channel-selected state |
...
- ✅ User-panel create = **Open only** (no invite system for Private).
...
- **Invite system for Private**: would need an invites table + `JoinChannelAction` accepting an invite token; today Private = owner-only.
