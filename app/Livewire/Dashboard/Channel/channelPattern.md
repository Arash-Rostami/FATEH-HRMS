...
**Gotcha (hook-caught):** `markSeen(id)` must run **inside `.then()` after success**, not synchronously before the async `$wire…joinChannel()` resolves — otherwise a failed join silently loses the invite (toast gone, id persisted as seen). `createChannel` follows the same pattern (marks `activeChannelId` seen only after `refreshUnread`). Both chains end with `.catch(()=>{})`.
...
├── invite-toasts.blade.php            ephemeral "added to channel" toast list (root-level, outside islands)
├── mention-toasts.blade.php           persistent mention-toast list (root-level, outside islands; §21)
...
- `mentionMemberMap(): array` — §21 (one indexed `memberUsers()->select('users.id','users.name')` query → `{name => [ids]}`; guarded on `$this->activeChannel`).
- `mentionToasts(): array` — §21 (one bounded unread-mention query across the user's channels; rendered as hidden `[data-mention-toast]` nodes in the sidebar island).
...
| `selectChannel(int)` | guard: `withoutTrashed()->whereKey($channelId)->whereHas('memberUsers', fn($memberQ)=>$memberQ->where('users.id', auth()->id()))->exists()` → set `activeChannelId`, `mobileShowChat=true`, `browseMode=false`, `createMode=false`, reset `loadedLimit`/`hasOlder`/`focusAnchorId`/`focusOlder`/`editingMsg`/`lastDeleted`/`messageSearch`, `composer->reset()`, `edit->reset()`, `markRead`, `unset(channels,messages,activeChannel,mentionMemberMap,mentionToasts)` | the membership guard is mandatory; silent `return` on fail (no 403 to client); re-selecting same channel exits anchor mode; uses `whereHas('memberUsers', …)` not `wherePivot` (wherePivot errors inside whereHas — see §11). Clears `browseMode`/`createMode` so the root `@elseif($browseMode)` branch (§10) can't shadow a just-selected channel after join-from-browse |
...
| `channel(array $c, int $authId): array` | `{id,name,slug,slug_handle,description,type,initials,unread,is_entered,last_message{body,time,datetime,is_mine}}` |
...
| `empty.blade.php` | | no-channel-selected state |
| `invite-toasts.blade.php` | | root-level `x-for="t in inviteToasts"` ephemeral toast list — included outside both islands (parent scope), same reason as `manage-members.blade.php` in §3.7 |
| `mention-toasts.blade.php` | | root-level persistent mention-toast list (§21) — also outside both islands; `channel.js syncMentionToasts()` reads hidden `[data-mention-toast]` nodes rendered in the sidebar island |
...
| `sidebar-row-actions.blade.php` | `@include` | both sidebars — per-row pin + mute buttons (`$store.pinned` / `$store.sound`, scoped by `'channel'`/`'contact'`); hover-revealed inside the row (`<div role="option">`, NOT a `<button>`, so nested interactive children are legal) |
...
Ephemeral toasts ("saved", "copied") already centralize in `components/ui/modals/toast.blade.php` (listens on the `window 'toast'` `CustomEvent`; `chatBase.toast()` dispatches it). Channel invite toasts are the documented exception — root-level `invite-toasts.blade.php` tied to Livewire state, outside both islands (§3.7). `quote-chip` is a text-selection reply bubble, **not** a toast.
...
- **Persistent mention-toast (client-side, NOT a Filament nudge/bell):** the only server touch is `Main::mentionToasts()` `#[Computed]` — one bounded query: unread messages across the user's channels (`leftJoin channel_members`, `id > COALESCE(last_read_message_id,0)`, `sender_id != auth`, `body LIKE '%<escaped-name>%'`, `limit(200)`), PHP-confirmed with the exact render regex, latest per channel. Rendered as hidden `[data-mention-toast]` nodes in the sidebar island (so they refresh on the EXISTING 10s `refreshUnread` poll — no new endpoint/cadence). `channel.js syncMentionToasts()` reads those nodes, filters by a localStorage `channel-mention-seen = {channelId: lastMentionMsgId}` cursor, and shows one toast per channel (mirrors `invite-toasts.blade.php` visuals). Click → `openMentionToast` → `selectChannel` + `scrollToMessage(id)` (loads the anchor window if the message is outside the recent view); dismiss/open advances the cursor. Opening the channel runs `markRead` → server cursor passes the message → it drops from the unread set → toast clears on next poll. NO new server-side mention state — the existing read-cursor IS the "seen" signal.
...
- **Test convention:** `tests/Feature/Livewire/Dashboard/ChannelMentionsTest.php` — presenter unit cases (wrap, link-order, departed, Persian, metachar, longest-name) + Main integration (`mentionMemberMap` duplicate-name ids, `mentionToasts` unread→read clear, self-send no-toast). useMysql + transaction + Queue::fake, mirroring `ChannelTest`.