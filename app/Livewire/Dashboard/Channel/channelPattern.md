...
- **Sanctioned nudge — `ChannelInviteNudge`, key `channels-controller:nudge`** — for invited-not-entered members, routed through the proper `:nudge` pipeline (bell surface only, no statemenu dot, no bare badge key). The enter path dispatches `ReconcileNudge` from `MarkChannelReadAction` (sets `entered_at`, prunes the just-entered user's row); the add/remove path dispatches from `SyncChannelMembersAction` (creates rows for newly-added-not-entered members, prunes removed members via `for()` recompute); the self-leave path dispatches from `LeaveChannelAction` (prunes the leaving user's row). Self-join (`JoinChannelAction`) does NOT dispatch (self-entering sets `entered_at=now`). The `Channel::deleted`/`forceDeleted` Eloquent binding prunes the channel's whole nudge row set when the channel itself is removed. The in-sidebar "not entered" dot (§10) is a **render flag from the existing query** — same category as the in-sidebar unread pill — NOT the HasMenuState/menu-badge pipeline.
...
- ❌ Don't add `HasMenuState` / a bare-key badge / wire:poll / a cache layer / a nested Livewire component — none belong here. The sanctioned nudge is `ChannelInviteNudge` (`channels-controller:nudge`, §3.6); no OTHER nudge/badge may be added.
