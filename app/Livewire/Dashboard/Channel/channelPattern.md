...
| `saveEdit(action, int)` | `$action->execute(edit, editingId)` → if false toast "مهلت به پایان رسیده" | edit window is 600s |
...
| `header-actions.blade.php` | `@component` + `@slot('sound')` | both headers — 4 identical buttons (search/highlight/maximize/info) + a `sound` slot for the per-conversation mute button (the only consumer-specific part) |
...
| `message-actions.blade.php` | `@include(['msg' => $msg])` | both `messages.blade.php` — hover copy/reply/edit/delete toolbar; param-free — the edit-button guard is inlined (`can_edit && $msg['id'] === $this->lastMessageId`), matching the delete gate (`can_delete && $msg['id'] === $this->lastMessageId`); both absolute-last, in sync (§12.2/§12.7) |
