...
- **Unread** = messages where `recipient_id = me`, `read_at IS NULL`, `deleted_at IS NULL`, counted per contact (`unreadSub` in `FetchContactsAction`, already scoped to `visibleOnBoard()` senders via its base query). The two **sitewide** unread counters — `Message::hasUnreadFor()` (hamburger-menu dot, `UnreadMessages` indicator) and `Message::totalUnreadFor()` (the messaging-tab numeric pill, `FetchMessagingUnreadAction`) — are separately scoped the same way (`whereHas('sender', fn($q) => $q->visibleOnBoard())`, fixed 2026-09-04): before this fix, an unread message from a sender who later went inactive or Guest-type was invisible in the sidebar (so unreachable to mark read) yet still counted forever by these two, producing a "badge says unread, nothing to open" symptom.
- **Marking read** = `MarkMessagesAsReadAction` sets `read_at` on the contact's inbound messages to me, then explicitly dispatches `ReconcileNudge('contacts-controller:nudge', User::class, $contactId)` (a mass `Builder::update()` fires no `updated` Eloquent event, so the sitewide bell nudge — `App\Services\Menu\Notifications\ContactNudge`, see `App\Services\Menu\statePattern.md` — would otherwise never clear; Channel's `MarkChannelReadAction` is the precedent for this explicit-dispatch pattern; fixed 2026-09-04).
...
**Nested anonymous Blade components (`<x-...>`) inside an island do NOT inherit `$__livewire`.** So `@entangle` / `<x-ui.forms.search>` (which needs `$__livewire`) fatals inside an island. The sidebar search reuses the shared `<x-ui.forms.search-field>` component (`components/ui/forms/search-field.blade.php` — promoted from the `livewire.dashboard.messaging.search-field` partial on 2026-09-14 when Project became the third non-messaging consumer; all four consumers — channel/contact/project sidebars + in-chat `messaging/search.blade.php` — use the component tag) — island-safe: plain `wire:model.live.debounce` + Alpine clear + `x-teleport` overlay + `wire:ignore.self`; the `refreshSidebarOnClose` prop appends `$wire.$island('sidebar').refreshUnread()` to its close handler. **Alpine-state contract:** the component binds component-level Alpine state named `{model}Fullscreen` and `{model}Value` (e.g. `model='search'` → `searchFullscreen` / `searchValue`; `model='messageSearch'` → `messageSearchFullscreen` / `messageSearchValue`). The consumer's Alpine data object MUST declare both, or Alpine throws `... is not defined` on every island morph — which silently breaks the morph and stops new content (e.g. a sent message bubble) from rendering. `contact()` declares `searchFullscreen`/`searchValue` (sidebar) and `messageSearchFullscreen`/`messageSearchValue` (in-chat `messaging/search.blade.php`), mirroring `channel()` 1:1 (both via the `chatBase` mixin); `project()` declares them directly in `project.js` (no chatBase) — this exact omission silently killed Project's sidebar fullscreen search until fixed 2026-09-14.

### 3.2 Polling (D1 — why Contact's poll differs from Channel's)

**Contact and Channel both now poll two islands (2026-09-12 — historically Channel polled only the sidebar; that asymmetry was closed, see `resources/js/scriptPattern.md`'s "Adaptive poll scheduler" section for the full reasoning).**

`contact.js` uses the shared adaptive scheduler in `mixins/chatBase.js` — a self-rescheduling `setTimeout` chain (not a flat `setInterval`, no `wire:poll`), 10s floor, backing off to 20s while a conversation is open / 30s when idle on the sidebar, snapping back to 10s on any real change or user activity (send/select/type). Each tick:
...
**Why both:** Contact's open 1:1 conversation must reflect a new incoming message during the poll. The `activeUserId` guard avoids a wasted re-render when no conversation is open.

`visibilitychange` gates polling: `document.hidden` → `stopPolling()`, visible → `resetPollInterval()` + `startPolling()` + one instant catch-up `_pollTick()`. Both are wired in `init()` and torn down in `destroy()`.
...
- **No `HasMenuState` / bare-key badge added by Contact itself** — unread is surfaced only via `data-total-unread` on the sidebar `<aside>`, read by `contact.js`'s `MutationObserver` (`syncPushNotify`) to fire browser push notifications. Don't add a *new* nudge/badge row to `notifications` from within this module — see `feedback_no_nudge_badge_proposals`. This does NOT cover `App\Services\Menu\Notifications\ContactNudge` — a pre-existing, separate sitewide bell layer (`contacts-controller:nudge`, see `statePattern.md`) that already exists outside Contact's own architecture; `MarkMessagesAsReadAction` correctly reconciles it (§13.11), that is a bugfix to existing infra, not new badge surface.
...
| `#[Locked] ?int $activeUserId = null` | selected contact (the 1:1 peer); Locked = client can't set it directly, bypassing `selectContact()`'s visibility check (2026-09-12) |
...
| `focusMessage(int): bool` | delegates to `FocusMessageAction::execute(activeUserId, id, authId, messagesLimit)` → `?bool`; `null` → return false; `false` → clear stale anchor + `invalidateMessageCache` + `dispatch('record-focus', type:'message')`; `true` → set `focusAnchorId=id`+`focusOlder=5` + `invalidateMessageCache` + dispatch | #[Locked] `focusAnchorId`/`focusOlder`/`activeUserId` (Locked 2026-09-12 — closed a gap where a raw client set could bypass `selectContact()`'s visibility check) — client can't tamper; anchor can't escape the active peer; reuses the global `record-focus` standard (`scrollToRecord('message-{id}')`) |
| `refreshUnread()` | `unset($this->contacts)` | called every adaptive-poll tick (sidebar leg); cheap recompute |
| `pingTyping()` `#[Renderless]` | `Cache::put("typing:dm:{me}:{activeUserId}", true, +8s)` | client-side leading-edge throttle (≤1 call per 3s while typing, via `chatBase.js`'s `pingTyping()`) — no request per keystroke; portable across any cache driver, see `scriptPattern.md`'s typing-indicator section |
| `peerTyping(): bool` `#[Computed]` | `Cache::get("typing:dm:{activeUserId}:{me}")` | rendered in the contact header in place of "last seen"; a `data-typing` DOM marker lets the poll tick treat "peer started typing" as a change, snapping back to the fast poll floor |
| `refreshActive()` | `invalidateMessageCache()` (unsets `messages` + `lastMessageId`) + `unset($this->activeContact)` (2026-09-12) | called every adaptive-poll tick (messages leg, only when `activeUserId` set — §3.2 D1); the `activeContact` unset piggybacks the peer's presence dot/last-seen text onto the same already-running tick — free live "online" status with zero new requests, only scoped to this one poll-triggered method, not the other 8 `invalidateMessageCache()` call sites |
...
| `markRead(int) #[Async]` | `app(MarkMessagesAsReadAction)->execute($userId, auth()->id())` | `#[Async]` = fire-and-forget from the client; called internally by `selectContact` (runs sync server-side); Channel parity; the action also reconciles the sitewide bell nudge (§2, §13.11) |
...
| `MarkMessagesAsReadAction` | `execute(int $contactId, int $viewerId): int` | sets `read_at` on the contact's inbound messages to viewer; dispatches `ReconcileNudge('contacts-controller:nudge', User::class, $contactId)->afterCommit()` to clear the sender's stale bell notification (§13.11); returns count |
...
- `EditMessageForm`: `editingBody` (`#[Validate('required|string|min:1|max:2000', onUpdate: false)]` — `textarea` binds `wire:model.live="edit.editingBody"`; without `onUpdate: false` the `required` rule fires on every keystroke and flashes the error the instant the field is cleared mid-retype, same premature-validation bug as `MessageComposerForm`/`ChannelMessageComposerForm`/`TaskBoard\Forms\ReplyForm`). `reset()` after save.
...
- `messages(array $messages, int $authId, int $editTimeLimit, ?int $firstUnreadId = null): array` — per-message view row (is_mine/is_last/can_edit/can_delete/sender/avatar/reply); adds `is_new_messages` (flags exactly the row whose id === `firstUnreadId` — zero new SQL, pure array transform) and `type_flags` (`BuildsChatBubbles::typeFlags()` — content-type letters for the client-side chat type filter, 2026-09-14).
...
|---|---|---|
| `contact.blade.php` (root) | `x-data="contact()"`, **outside** both islands | two `@island` blocks; root-level: keydown handlers, `<x-ui.modals.max-backdrop/>`, quote-chip, switch-tabs, title; messages island branches `@if($this->activeContact)` → header+messages+composer+info · `@else` → empty — each branch root is a `wire:key`-ed `class="contents"` wrapper (`pane-active`/`pane-empty`, 2026-09-14) so morph swaps whole branches as units (unkeyed branch morphing left stale empty-state divs in the DOM — the "lingering shell at mobile width" flake; see channelPattern root entry) |
| `sidebar.blade.php` (sidebar island) | `@php($p = $this->presenter; $contactList = $p->sidebar($this->contacts, auth()->id()); $totalUnread = $p->totalUnread($this->contacts))` | `data-total-unread="{{ $totalUnread }}"` (push-notify); shared `livewire.dashboard.messaging.search-field` partial (`refreshSidebarOnClose` → `$wire.$island('sidebar').refreshUnread()`); filter tabs; row `x-on:click="selectContact(id)"` + enter/space |
...
startPolling()      chatBase adaptive scheduler (10s floor, 20s/30s ceiling) → $wire.$island('sidebar').refreshUnread().catch()  AND  if ($wire.activeUserId) $wire.$island('messages').refreshActive().catch()   (D1 dual-island — see §3.2; NO wire:poll)
...
backToList()        (added 2026-09-23, Channel-parity) clear searchMessages/replyingTo/editingMsg/deletingId/openActionsId; optimistic $wire.set('mobileShowChat', false, false) (local-only, zero request — §13.13); $wire.$island('messages').backToList().then(() => $wire.$island('sidebar').refreshUnread())  — previously the header button was a plain wire:click="backToList" (auto-scoped to the messages island only), so the sidebar never re-rendered until the next poll tick (§13.12)
...
- ✅ Reuse the shared `livewire.dashboard.messaging.search-field` partial for the sidebar search — it is island-safe (plain `wire:model` + Alpine, no `@entangle`).
...
- ❌ Don't add `HasMenuState` / a bare-key badge / a *new* nudge type to `notifications` from within this module — unread is surfaced only via `data-total-unread` + the push-notify observer. (The pre-existing `ContactNudge` bell layer is separate infra, not covered by this rule — see §3.4/§13.11.)
...
3. **`@entangle`/`<x-ui.forms.search>` in sidebar island** — fixed by reusing the shared `messaging/search-field` partial (plain `wire:model` + Alpine, no `@entangle`).
...
11. **Stale bell nudge after reading a conversation (fixed 2026-09-04).** `MarkMessagesAsReadAction`'s `read_at` write is a mass `Message::withoutTrashed()->...->update(...)` (Eloquent Builder, not a model instance) — mass updates fire **no** `updated` model event, so `App\Services\Menu\Notifications\ContactNudge`'s `Message` `updated` trigger (registered in `NudgeServiceProvider`) never reconciled on read. The bell's per-sender "N unread messages" notification (`contacts-controller:nudge`) stayed unread forever, even after the conversation was fully read and the sidebar dot/`data-total-unread` had already cleared correctly (those go through `StateService::flush()`, which the action always called). Fix: the action now also explicitly dispatches `ReconcileNudge('contacts-controller:nudge', User::class, $contactId)->afterCommit()`, the same explicit-dispatch pattern `MarkChannelReadAction` and `ShareEventAction` use for their own mass/bulk writes (see `App\Services\Menu\statePattern.md`). Do not remove this dispatch thinking it's a new nudge/badge addition — see §3.4/§12.
12. **Mobile `backToList()` never had an Alpine wrapper (fixed 2026-09-23) — the sidebar only reappeared on the next poll tick.** Unlike Channel, `contact/header.blade.php`'s back button was a plain `wire:click="backToList"` — per `livewirePattern.md`'s `@island` rules, that auto-scopes the response to whichever island it's rendered inside (`messages`), so `Main::$mobileShowChat = false` was set correctly server-side but the `sidebar` island (which reads `$mobileShowChat` to toggle `hidden`/`flex` — §10) never got told to re-render. The list only reappeared once the next adaptive-poll tick (§3.2, 10-30s) happened to refresh the sidebar island. Fixed by adding a `backToList()` wrapper to `contact.js` (mirroring Channel's, §11) that does `$wire.$island('messages').backToList().then(() => $wire.$island('sidebar').refreshUnread())`, and switching the button to `x-on:click="backToList()"`.
13. **Optimistic client-only `mobileShowChat` flip for instant back-navigation (added 2026-09-23).** Even island-scoped, `backToList()` is still a real network round-trip before the pane swap paints. `contact.js`'s `backToList()` now also calls `this.$wire.set('mobileShowChat', false, false)` as its first line — the 3rd `false` arg (verified against `vendor/livewire/livewire/dist/livewire.esm.js`'s `$set` wire-property, aliased as plain `.set`) updates only the Alpine-reactive local mirror, firing **zero** network request. `contact.blade.php`'s `<main>` and `contact/sidebar.blade.php`'s `<aside>` both gained a parallel `x-bind:class="{...}"` reading `$wire.mobileShowChat` (object-form, so it explicitly reconciles against the pre-existing static `@class` on first paint — no risk of `hidden`+`flex` both landing) alongside the untouched server `@class`, so the pane swap now paints instantly while the real `$island()` calls still run in the background for the actual state reset. Scoped to `backToList()` only, not `selectContact()` (per explicit user request) — this is a deliberate, safe asymmetry (nothing else keys off client/server `mobileShowChat` lockstep). **Known residual risk (reviewer-flagged, not fixed):** no request-cancellation exists for overlapping `$island()` calls on the same component — a `backToList()` tap immediately followed by a `selectContact()` tap, on a slow connection, could theoretically have the `backToList()` response land *after* `selectContact()`'s and stomp its state back to "list view." Narrow, low-probability, UI-only; revisit only if actually observed.
...
- **Shared `messaging/search-field` partial** — sidebar search reuses the same partial as Channel; Alpine-state contract `{model}Fullscreen`/`{model}Value` declared in both `contact()` and `channel()`.
