...
**Indexes:** the window only helps if `created_at` is indexed — each high-volume table's `idx_<table>_created_at` index is baked directly into its `create_<table>_table.php` migration under `database/migrations/migrated/` (`tickets`, `dms`, `channel_messages`, `messages`, `reservations`; feeds/posts already had one). When adding `$recencyMonths > 0` to a new resource, check `SHOW INDEX` for its table first and add the index to that table's own create migration if needed.
