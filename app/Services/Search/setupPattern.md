...
- **Scope** — the user's **own sent messages** only: `where('sender_id', $me)` + `whereHas('channel', fn $q => $q->whereHas('members', fn $q2 => $q2->where('user_id', $me)))`. The `sender_id` clause excludes other members' messages; the membership clause excludes any channel the user is no longer a member of (so a hit's deep-link always opens). No org-wide scope, no non-member channel content. (The in-chat channel search is deliberately broader — it searches the whole conversation once the channel is open.)
