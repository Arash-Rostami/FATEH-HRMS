...
## MANDATORY morph-safety rule for all Livewire views (since 2026-09-14)

Every conditional branch (`@if`/`@elseif`/`@else`, `@forelse`/`@empty`) that can flip on user interaction (live search, `wire:click`, island refresh) MUST carry a `wire:key` on its root: directly on a single-root element, or via a `<div wire:key="..." class="contents">` wrapper for multi-root branches (never a `div` at `<tbody>`/`<tr>` level — key the `<tr>`). Keys are static, kebab-case, module-prefixed; branch keys never carry row ids; branches rooted in `<x-ui.empty>` (which swallows `$attributes`) always get the wrapper. Full rules + rationale: the "Morph-safety" section in `app/Livewire/livewirePattern.md`. Applies to every new or modified user-panel module, admin Livewire views included.
